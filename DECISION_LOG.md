...
## 2026-09-15 · Group review after corrections: one rule for the union and the helpers, simulated boards for every group, Problem Set 6 regenerated (ticket 332)

**Decision.**
- **One rule, one copy.** `lib/reviewUnion.ts` decides a member's questions for their group (`stillToReview`: not right first time and not fixed in individual review, or first submissions on a pathway without it) and who can explain one (`canExplain`: right first time, or fixed). A fix is a second submission with no wrong line, the report's own "Fixed in individual review", so no report shows a student fixing a question their group then had to work because of them. The live board, the standings, the simulated groups, the stage's done count and the record-review checks all read it.
- **A group with nothing left sits out.** It is never listed (`groupsAt`), so every count of groups follows (the teacher's card, the race, the leaderboard, the stage count, where its members count as done). The live student's board begins empty (the race keeps its clock) and his iPad goes on to the stage after group review with "Your group has nothing left to review."
- **Every group's board plays in full.** The four groups that are not Sam's run simulated boards (`lib/groupSim.ts`) that replay the whiteboard's own reducer (`groupReducer`, moved to `lib/groupReview.ts`) at a per-group pace, pens by the product's shuffle, so a simulated run is a `GroupRun` like the live one. `RACE_SCHEDULE` is gone; the progress card now shows every group's left-for-now and unsolved questions. The tries a table writes are chosen by the rule for who is present (`boardScripts`): authored tries where they fit, else the rule's own shape from the table's real slips. The live board carries its chosen tries in the run (`GroupRun.scripts`), so if the presenter finishes Q9 in individual review, Sam can explain it and the board solves it in two tries.
- **Problem Set 6's data.** Seven second submissions slip again (Q7 for Jordan, Zara, Isla, Lucas, Oliver and Ruby; Zara's Q9). That gives the outcomes Carson asked for: Q7 left for now then unsolved at mint, sky and violet, mint's Q10 unsolved, sky's Q9 left for now then solved on its return. The story sheet's Set 6 review, the group versions and the class review picks follow; class review still covers Q7 and Q10.
- **Scoping.** The record check runs ticket 332's rule on Problem Set 6 only (`AFTER_REVIEW_RULE_SETS = [6]` in `lib/reviewRule.ts`, removed by ticket 338).
- **The gate.** The classmates' arrivals take about a minute (last at 59.1 s), named simulation data in `data/arrivals.ts`; the product rule (open once everyone in the room is in) is unchanged.

**Context.** Carson, 2026-09-15: "if every member of the group has a problem right after indiv review, doesn't count in union of mistakes. otherwise, we jeopardize making indiv review seem pointless." The helper rule and the sit-out were settled in the review-control grilling and the 318/319 design session the same day, with a starting outcome list (mint Q7, violet Q7 and sky Q7 left for now then unsolved; mint Q10 unsolved; sky Q9 left for now then solved). Ticket 319's per-group grid needs every group's turns, not just sky's.

**Alternatives considered.**
- *Keep the second submissions and pick other questions for the outcomes.* This follows the brief's fallback literally. Under the old second submissions, Jordan, Zara, Isla, Lucas, Oliver and Ruby all fixed Q7, so every table would have a helper on Q7. Only mint's Q10 would be left for now and unsolved anywhere, and only sky and mint would have a question nobody can explain. Rejected: it loses most of what Carson asked for. Q7 is the class's hardest fraction question, and Sam's own rework slips on it again, so a second slip is realistic.
- *Change first submissions to give coral and amber a question nobody can explain.* Rejected: set scores must not change, and Priya (coral) had all ten right while Noah and Mia (amber) cover every question between them.
- *Keep a scripted percentage race for the other groups.* Cheaper, but it cannot say whose turn it is or which question was left for now, which ticket 319's grid needs, and it could disagree with the rule.
- *Hand-author every simulated check moment.* Exact control, but the ladder (third wrong check, pause, return after the union) would have to be kept consistent by hand. A pace per group plus the reducer keeps it consistent by construction.
- *Make Sam's Q9 rework wrong so he can never explain it.* A wrong rework of a problem whose first submission held trips the individual-review guard and blocks hand-in, so the presenter could not continue. The board adapts to the rule instead.
- *Count a sitting-out group as a 100 % row.* Keeps five rows, but Carson said five with one sitting out reads as four.

**Tradeoffs.**
- **Per-second cost.** The standings replay four simulated boards from their start on every call (about 50 reducer steps each). This is cheap now; memoise if boards grow.
- **More moving parts.** The run carries simulation-only tries (`scripts`) beside the simulation-only pens, and reading a run from before this ticket falls back to `GROUP_SCRIPTS`.
- **Seven rewrites are data, not a rule.** The story rule "a one-off is fixed on the student's own rework" becomes "a one-off is rewritten, and fixed only if the rewrite holds", so the checks can no longer say which one-offs must hold.
- **Past sets still use the old rule.** Problem Sets 1–5 keep ticket 278's outcomes until ticket 338, so the reports mix the two rules for now.
...
## 2026-09-16 · Changing the pathway live: locked once entered, written by the decision's answer, routed at each transition (ticket 336)

**Decision.**
- **The rule is pure and per student.** `lib/pathwayChange.ts` puts every student in the room on the line: working 0, individual review 1, group review 2, class review 3, done 4. Sam comes from his session's stage, with the gate as group review's entry and the wait as class review's. Each classmate comes from the demo's own model of the class: working until the stream hands them in, then at the entry of the stage the pathway gives after hand-in, after their corrections, or after their group is home or sits out. The class-wide moments raise everyone: the gate opening, the board projected, the lesson over. A review on the pathway is locked once anyone present is at its place or past it. A review that is off has been entered by nobody, so it can always be switched on. Absent students never lock anything. Individual working is always locked.
- **Done resolves at the press.** The card shows the locks from the teacher's 3-second session batch. Done reads the classroom, Sam's live session and the clock again, and keeps any review a student entered while the teacher was choosing as it was (`changedPathway`). The answer is `change` with that pathway, or `keep` when the result is the pathway as planned.
- **The answer writes the pathway.** `DecisionAnswer` gains `{ kind: "change"; pathway }`. The classroom reducer writes `answerPathway(answer)` to `assignment.pathway` in the same `decision/answer` step, only when that step answers the decision (the first answer stands) and the pathway is valid. No separate pathway action exists, so 337's split can add class review by giving its own answer a `pathway`.
- **Routing needed no change.** The student tab already passes `pathwayOf(classroom)` to the session reducer on every dispatch, so hand-in, corrections handed in, group review done and the teacher's force submits all read the pathway as it is. The strips read it through `pathwayStages`.
- **Groups from seating.** `groupPlan` now takes the set's seating, and `liveGroupPlan(c, session)` passes the set's own groups (`assignmentGroupsOf`, frozen at Create from the class defaults whether or not group review was planned), its absences and the rule for the pathway as it is. Sam's live board and the gate were the one place still reading a fixed list of three groupmates. The other groups already came from the seating (`groupsAt`).
- **The card's line.** The four stages stack top to bottom in the 400 px card, each a Create stop (`components/PathwayStop.tsx`, 136 × 32 layout px, 13.5 px) with its description beside it, joined by the line's ink track with an arrowhead into each stop. It uses the strip's short words, carries the hint "Switch any review students haven't started.", and has no "done" end stop. A locked stop's title says students have started it.

**Context.** Carson, 2026-09-14 (ticket 254, deleted): teachers "should be able to change during the actual ICW & lesson". The review-control grilling (2026-09-15) put the change on the decision card. The ticket fixed the rule ("while no present student has entered it"), the look (Create's line, locked in ink, Done), and routing "at their next transition".

**Alternatives considered.**
- *Lock by the class's stage (the strip's current pill), not per student*: simpler, and in the demo individual review would stay switchable until Sam hands in. But a classmate who handed in is on their corrections, and removing the stage under them misstates what the class is doing. Per student is what the ticket says.
- *Also lock a review that is off once students are past its place*: this avoids a class split between students who skipped a newly added review and students who get it. But it contradicts "entered", and a teacher adding class review late for the students still working is a real choice.
- *A `pathway/set` classroom action beside the answer*: more general, but two writes that could disagree, and a pathway change with no decision behind it has no screen. The answer carrying the pathway keeps one record of why the plan changed.
- *Enforce the locks in the reducer*: the reducer has no session or clock, and the teacher's tab is the only writer; resolving at the press with fresh inputs is as strong in practice.
- *Create's horizontal line inside the card*: five 156 px stops need 780 px; a vertical line keeps the look (stops, track, arrowheads, descriptions) in 400 px.
- *Re-freeze the groups from the class defaults when group review is switched on*: the set already froze them at Create, and its Groups tab (which appears with group review) edits that copy.

**Tradeoffs.**
- **The demo's window is short.** On a fresh lesson the card comes due at 3:34 and the first classmate hands in at 4:11, so removing individual review from the card has about 37 seconds (the click-through rewinds the set's start to reach it). From Sam's hand-in, Priya (nothing to fix) is done reviewing at once, so group review is locked for the whole of individual review.
- **A change made after some students passed a transition is not recorded per student.** Their reports read the pathway as it is now.
- **Adding a review adds its card.** Adding class review puts the Class review card in Class View's side column, and the Live diagnostic card below it moves down. Adding group review adds the Groups tab.
...
**Decision.**
- **The table, not the pen.** The cell a group is on draws every present member of that group (ticket 332's `members`, seating order, the absent already out), and the ring in the group's colour moves from the cell onto the pen-holder's avatar. The cell keeps a fainter inset ring in the same colour, so a glance across the grid still finds each group's question while the strong ring names the student working.
- **Sized to the cell, measured once.** `cellAvatars(avail, seats)` in `lib/groupGrid.ts` is the arithmetic: the avatar size, the gap between them and the initials' size that fit `seats` avatars, plus the pen-holder's ring at each end, inside a cell. `GroupGrid` measures its narrowest cell before paint (a layout effect, again on a resize and once the fonts land) and writes `--cell-av`, `--cell-av-gap` and `--cell-av-text` on the grid: no state, no re-render, and every cell draws at the one size so the tables read alike. Five groups at 1280 x 800 give about 115 px of cell, which puts four avatars at 24 px; the floor is 16 px, where initials stop being readable.
- **The gap clears the ring.** The pen-holder's ring is drawn outside their avatar, so the gap between avatars is 4 px while they are near full size (3 px once they shrink): at 3 px the ring sat a pixel from its neighbour.
- **`Avatar` takes a style.** `components/ui.tsx`'s `Avatar` gained an optional `style`, so a measured size can replace its size classes; nothing else about it changed, and every other caller keeps its classes.

**Context.** Carson, on ticket 319's grid (2026-09-16): "show all 4 student avatars in the box, with the colred ring around the student who's working." Group review is a table of four working one question together (three at amber, with Chloe absent), and the teacher walking the room reads the table, not only whose turn it is.

**Alternatives considered.** Keeping the ring on the cell alone and drawing the pen-holder first in the row: the ring then says where the group is but nothing says who is writing, which is what Carson asked for. Fixing the avatars at ticket 319's 28 px and letting a cell scroll or clip: four of them overflow a 115 px cell at 1280 x 800, and the grid must not scroll sideways. A CSS-only fit (`clamp` on a container query): the cell's width is a grid share, not a container the avatars can be sized from, and the fit has to hold under the teacher's 0.72 zoom, where client rects are in screen px.

**Tradeoffs.** The avatars carry initials only, so the teacher reads the seating rather than names; at 1280 x 800 there is no room for more. The fit is JavaScript, one more layout effect on a page that already has two (the place table's fold, `FitGrid`), and it runs on every render, as those do. A table larger than four in a class seated in six or seven groups would hit the 16 px floor and is deferred.

**Defense.** The cell now answers both questions the teacher has while crossing the room: which question is this table on, and whose turn is it. The arithmetic is pure and unit-tested, so the fit is checked without a screenshot, while the click-through still measures the avatars against their cell at both laptop sizes.

## 2026-09-16 · The split of group review and class review: the suggestion the class's own corrections make, one press, one record (ticket 337)

**Decision.**
- **One pure module.** `lib/splitReview.ts` counts how many of the room have each question right and says what to suggest. Once the class is in individual review a question is right for a student when it was never theirs to fix or their correction is in and right — Where students are' own rule (`lib/reviewPlaces.ts`), so the card and the Mistakes column can never disagree. While the class is still working (the pathway variants, where the split rides on ticket 335's card) the count is first submissions so far, and the card says so: counting corrections before individual review has happened would read the class's finished work, which is a projection. The suggestion is the two questions the fewest have right (ties broken by the later question of the set, the harder end), every other question below half offered unticked as "also often wrong", and the rest behind "all questions".
- **When it comes.** During individual review, once half or more of the students in the room have handed their corrections in (the count the strip's "n/19 done" shows), and only while something is below half. It lapses when the class leaves individual review — the gate never waits for it. On a pathway without individual review there is no such moment, so the split rides on ticket 335's close-to-finishing card; on one without group review the card says plainly that the questions most got wrong go to class review.
- **Frozen at the moment it came due.** The counts and the ticks are read at `dueAt`, not at `now`, so a row never moves and a tick never changes under the teacher's cursor while corrections keep landing.
- **One press, one record.** The answer is `{ kind: "move", moved, pathway? }` on the decision (`lib/decisionState.ts`), and `movedToClassReview(c)` is the one accessor every reader goes through: the group unions (live and simulated), the race, the progress card, the stage's done count, ticket 319's grid, class review's setup and the reports. A move always leaves class review on the pathway, so a question taken out of group review is never dropped; the reducer writes that pathway in the same step as ticket 336's change. A move is refused once a board has opened, so no group's questions change under a running board.
- **After the press the card stays.** "Saved for class review.", what leaves group review, and **Set up class review →** beside **Close** — the setup itself (the examples) stays on its page, where a moved question is ticked, cannot be unticked, and is marked "from group review".
- **When nothing would be left.** If the ticks would empty every group's list, the card says so and its button reads "Skip to class review"; group review is locked by then in the demo (a student is at the gate), so the skip happens as every group sitting out (ticket 332), each iPad going on with "Your group has nothing left to review."
- **The nearby fix.** On a pathway without individual review the class waits at the gate into group review with the working still current, counting the students in at the gate, and its force submit opens the gate (`atGateWithoutReview`, `forceKind`). It used to read "19/19 done" with force submit greyed out.

**Context.** Carson, review-control grilling 2026-09-15: "teacher may only want to have select questions be a part of [group review]. say for instance only 5 students got to Q10 … & save Q10 for class review … don't want to have them make that decision twice … WE need to make the suggestion." The ticket fixed the trigger, the counts ("correct counts corrections"), the two pre-ticks, the "also often wrong" list, the whole-question rule on every row, and that every question ends in exactly one of group review and class review.

**Alternatives considered.**
- *Count what the class will have right at the end (the fixtures' second submissions).* The suggestion would be stable, but it is a projection of work not yet done; the review-control entry already rejected projections. Counts as they stand are honest, and by the trigger half the room's corrections are in.
- *Live counts on the card.* Truer by the second, but a row could climb past another while the teacher reads, moving the ticks; the decision lives for about half a minute, so freezing at `dueAt` loses nothing real.
- *A per-group split (move a question out of one table's list).* Finer, but it is a decision per group per question; the review-control entry settled on one decision for the class.
- *Let the teacher move a question and leave class review off.* Rejected: the question would be dropped. Accepting adds class review and the card says so.
- *Block the gate until the card is answered.* Rejected in the review-control entry: ignoring the card keeps the plan.
- *A separate `wc/lock` action for class review's pre-ticks.* Rejected: the setup page reads the answer, so there is one record of the decision and nothing to keep in step.
- *At the gate without individual review, make group review the current stage.* The gate is group review's entry on the line (ticket 336), but the strip shows the stage students are still finishing, exactly as individual review stays current while its gate fills. Keeping the working current also keeps its force submit, which is what opens the gate.

**Tradeoffs.**
- **The window is about half a minute** in the demo: the card comes when ten of the nineteen have handed in corrections and the gate opens when all nineteen are in. That is the ticket's design (the gate never waits), and the click-through holds Sam's arrival back to work in it.
- **The counts and the "n need to fix" column can drift apart** once the card has been up for a while: the card shows the counts as they were when it came due.
- **The card grows.** With "all questions" open the list scrolls inside the card rather than the card growing past the scroll region.
- **A moved question's report tile** reads "Covered in class review" with no group version. A finished set's records still carry group versions for questions that never reached a group; nothing shows them once the set is finished under a move, but the fixture is now read through the moved list.
- **The strip's count at the gate drops** on a pathway without individual review: the stream hands every classmate in at Sam's hand-in, while the gate's arrivals take a minute, so "18/19 done" becomes "1/19 done" and climbs again. The gate's count is the honest one (it is what the iPads and the gate itself read); the stream's jump is the demo's.

**Defense.** The suggestion is made from the same numbers the teacher is already reading on Mistakes, at the moment the evidence exists, and accepting is one press. One stored answer carries both what moved and the pathway it needs, so every screen that counts groups or questions — the boards, the race, the grid, the setup page and the reports — reads one list through one accessor and cannot disagree. The questions the class could not do end up where a teacher can teach them, and nothing is dropped on the way.

## 2026-09-16 · Coral's unsolvable question: Q8, three real first submissions changed, the group's slip written twice (ticket 347)

**Decision.**
- **Q8, not Q4/Q5/Q9, and not Q7/Q10.** Ticket 332 could not give coral a question nobody at the table could explain because Priya had all ten right first time. Ticket 347 asks for exactly that, changing as few first submissions as Carson's "just for one of them" allows. Q7 and Q10 are already handled by ticket 337's split (out of bounds by this ticket's own brief). Of the remaining candidates in coral's union (Q4, Q5, Q8, Q9), all four need the same three members' first submissions changed (Priya unconditionally, since she is the class's only always-right student; Amelia and Aiden, since only Tomas ever needed these four). Q9 was ruled out after checking `talliesAfterCorrections` directly: giving Priya, Amelia and Aiden a Q9 mistake drops the class-wide Q9 tally from 10/19 to 7/19, crossing ticket 337's below-half line and pre-ticking Q9 for the class-review split, a real collision with a shipped feature. Q8 was chosen over Q4/Q5 because its evaluation table (`data/evaluation.ts`) carries exactly one recognised wrong line (`Q8_MIRROR`, "graph-signs") that no classmate used before, so reusing it needed nothing invented and touched no other student's existing story.
- **Three first submissions change, not one.** Priya is the class's only always-right student, so she alone stays an explainer for any question outside Q7/Q10/Q9 unless changed; with her changed, Amelia and Aiden remain explainers unless they change too. There is no way to give coral an unsolvable question outside Q7/Q10 with fewer than three changed first submissions. Priya's, Amelia's and Aiden's set scores move (10→9, 7→6, 9→8); nobody else's does.
- **The group's own version writes the one recognised line twice.** `data/recordReview.ts`'s rule (checked by `data/story.test.ts`) refuses a group's "last try" that is byte-for-byte a member's own first submission. Q8 has no second recognised wrong line to build a differently-shaped attempt from, unlike Q7's three distinct wrong turns or Q9's multi-line working. `CORAL_Q8_LAST` is `Q8_MIRROR` written twice — two turns at the board, still a real slip, just not identical in shape to any one member's one-line submission.
- **No first submission of Priya's, Amelia's or Aiden's outside Q8 changed, and amber's data is untouched.** Every other count that moved (the Mistakes tab's Q8 row, class review's coverage, the class story sheet's graphing cell, the simulated race's timing) is a read of these two data changes through code that already existed; nothing needed a new rule.

**Context.** Carson, 2026-09-16: "change a first submission & a score (just for one of them, say coral — keep amber)." `architecture/332.md` names the reason coral (and amber) could not have one: "Priya had all ten right first time... [ticket 332] never changes a first submission, because set score is the first submission (ticket 285)." Ticket 347 is the deliberate exception to that constraint, scoped to coral only.

**Alternatives considered.**
- *Pick Q4, Q5 or Q9 instead of Q8.* All three still need Priya's, Amelia's and Aiden's first submissions changed (the same cost as Q8), and Q9 additionally collides with ticket 337's split-review threshold. Rejected: no cheaper, and Q9 breaks a shipped feature's boundary case.
- *Flip only Priya, leaving Amelia and Aiden untouched.* Leaves two explainers at the table (Amelia, Aiden both right first time on Q4/Q5/Q8/Q9), so the group would still solve it in one or two tries. Rejected: does not produce an unsolvable question.
- *Make the group's Q8 "last try" identical to Priya's, Amelia's or Aiden's own submission.* Simpler, and would have been the ticket's first instinct given only one recognised wrong line exists. Rejected: fails the checked rule directly (`data/story.test.ts` catches it: "the group's last try is priya's first submission").
- *Invent a second wrong line for Q8 to give the group's board some variety.* Would let the last try look like Q7's or Q9's escalating disagreement. Rejected: an unrecognised line evaluates as "unclear", explicitly checked against, and inventing a slip nobody makes is exactly what ticket 347's brief forbids.
- *Retune coral's simulated pace to keep its old finishing slot in the race (just before sky's board).* Q8's ladder (three wrong checks, the pause, a failed return) costs coral about 72 more seconds, moving its finish from ~280s to ~352s — just past sky's ~344s instead of just before it. Rejected: the same mechanic already makes mint and violet finish later than sky; hiding it by retuning pace would be engineering around an honest, minor consequence rather than reporting it.

**Tradeoffs.**
- **Three students' scores move, not one.** The ticket's own problem statement hoped for "ideally exactly one first submission." The acceptance criteria anticipate more than one ("the changed students'" scores, plural), and the honest minimum given the Q7/Q10 exclusion is three.
- **Q8's unsolved board has less visible variety than Q7's or Q9's.** With one recognised wrong line, the group's last try is that line twice rather than a distinct escalating disagreement. A teacher clicking into it sees Priya's and Amelia's examples look the same in class review.
- **Coral's race position moves.** It no longer finishes alongside amber, ahead of the demo group's own board; it now finishes just after it. Still comfortably inside the twelve-minute cap, and mint and violet are still clearly the slow two.
- **"Priya is secure in every category on every set" is no longer literally true.** It was a foundational, explicitly tested fact of the whole six-set demo (`data/story.test.ts`, `specs/class-story.md`'s own rules section). It is now true everywhere except Set 6's graphing, which reads solid (one slip among many held graphing lines), and every place that stated the old blanket rule is updated to the narrower one.

**Defense.** The data change is the smallest the Q7/Q10 exclusion allows (checked by computing, not guessing, how many members would still explain each candidate question), it reuses a slip the class's own evaluation table already recognises rather than inventing one, and it changes nothing outside coral: amber, mint, sky and violet are byte-for-byte unchanged, and every downstream count (scores, the Mistakes tab, class review's coverage, the class story sheet, the race) is a read of the same two data files through code that already existed for exactly this shape, so nothing needed a new rule to agree.

## 2026-09-16 · The warm-up's way-on buttons pin to the corner; "Next skill" checks the warm-up is finished (ticket 349)

**Decision.** "Your turn" and "On your own" move from the end of scrollable content to `position: absolute; bottom/right: 1rem` inside a `relative` wrapper around the step's middle column (`app/student/screens/PracticeSteps.tsx`), so a long worked example (fractions runs seven steps) never pushes the way on below the fold. Both still act unconfirmed the moment they're pressed. "Next skill" is different: it can skip a student past unfinished work, so it alone is checked — greyed out (`aria-disabled`, not a true `disabled`, so the press still lands) until the current skill's alone-on-the-pad follow-up is finished (`scriptDone` against `warmupScript`), and a press before then raises a corner confirm naming the skill rather than leaving silently.

**Context.** Carson, from a screenshot of the fractions warm-up (2026-09-16): the worked example needed a scroll to reach "Your turn", which should instead "always tak[e] up the bottom right corner of that screen." Separately, "have 'next skill' greyed out until all 3 warm up stages are complete," with the confirm's exact wording given ("are you sure you'd like to move on from {skill}? you haven't finished the whole warm up"). A later message drew the line explicitly: "your turn" and "on your own" stay unconfirmed — "diff functionality from the 'next skill' thing."

**Alternatives considered.**
- *A `disabled` attribute on "Next skill" instead of `aria-disabled`.* Simpler, but a genuinely disabled button eats the click, so a student pressing it while unfinished would see nothing happen rather than the explanation the ticket asks for.
- *Reset the confirm with a `useEffect` keyed on the current skill/phase.* The natural first reach, but this repo's lint (`react-hooks/set-state-in-effect`) refuses a `setState` inside an effect body. Storing the *key the confirm was raised for* (`confirmFor`) and comparing it against the current key on every render gets the same "stale confirm drops on navigation" behaviour as a pure derivation, no effect needed — and, as a side effect of the comparison rather than a special case, it also drops the confirm if the student finishes the alone problem while it's still open, which an effect keyed only on skill/phase would have missed (that was caught by the click-through, not designed in up front).
- *Require the same finished-check for the ladder-less pad case (a practice with no worked/completion split; none exist in the current bank, held by a test) too, or leave it always enabled.* Chose the same rule for consistency, since the ticket's "haven't finished the whole warm-up" reads the same whether the skill has one step or three; flagged rather than silently assumed, since the case is currently unreachable and untested by real content.

**Tradeoffs.** The pinned corner button sits over the last few lines of whatever it's pinned above (the worked example's last step, the pad's last blank) rather than after them; at the sizes this screen runs it clears the content in every warm-up in the bank (checked against the seven-step ones: fractions, quadratic sketching), but a future problem with more lines than fits above `pb-24`'s reserved space would need the same padding bumped. The corner confirm has no backdrop (matching `HandInCheck`'s existing convention, ticket 115), so a student can keep writing behind it, which is what makes the "closes itself once finished" behaviour necessary rather than a nicety.
...
**Context.** The two "see the example again" entries sit one step apart in the same warm-up, reachable inside the same few minutes on the same skill. `ExamplePeek` itself is the thing that changed shape (compact, `FitHeight`-fit, left-column sized); leaving its second caller passing the old full-width, uncompacted expectations would have either broken stage 3's layout outright or required a second, near-duplicate peek component to keep it looking like before — neither is "leave it as it was."

**Alternatives considered.** *Fix `CompletionStep` only, leave `PracticePad`'s `exampleAgain` peek as a full middle-column swap.* Cheapest, but the two entries point at the same words ("see the example again") one screen apart and would behave in contradicting ways: works with the pad on stage 2, hides it on stage 3. *A second, stage-3-specific peek component that keeps the old full-column look.* Avoids touching `PracticePad.tsx`, but duplicates `ExamplePeek` for no product reason and leaves the same complaint ("can't see both at once") sitting unfixed one step later in the identical flow.

**Tradeoffs.** The ticket's diff is larger than the literal ask (a second file, `components/PracticePad.tsx`, restructured alongside `PracticeSteps.tsx`) for a request that named only stage 2. `PracticePad.tsx` also serves the mid-set isolated-practice overlay (`PracticeOverlay`, no `exampleAgain`) and the help-on-a-question ladder's pad; neither passes `exampleAgain`, so neither is touched by this change — verified by reading every caller before editing, not by assumption.

**Defense.** One component, one behaviour: "see the example again" now means the same thing everywhere the warm-up offers it, and the pad it hands back to is the same pad the student was already using. The alternative was a warm-up that visibly disagreed with itself one screen later.

## 2026-09-16 · The decision card confirms one stage, not the whole pathway; moving questions to class review gets a second look (ticket 351)

**Decision.**
- **The passive card shows only the next stage, not the whole `<ol>` of every stage with its description.** Carson, looking at ticket 335's card: "so overwhelming. just want teacher to confirm the NEXT stage — not the whole thing. if they need to see the sequence of stages, they have the reference in the top right corner." The current stage is a fixed, already-made decision the teacher cannot act on from this card; naming it added nothing but weight. One line survives: the next stage's name and its Create-authored description (`STAGE_DESCRIPTION`/`WHAT_HAPPENS`), reusing the exact words the toggle editor already shows. Change (ticket 336) is untouched — it still exposes every not-yet-reached stage, on purpose: that is a deliberate, opt-in look at the full sequence, not the default one.
- **A stage the pathway skips over is said as a plain fact, never left silent.** Where the next review stage in `REVIEW_ORDER` is not the one actually next (individual review off, or both individual and group review off), the line reads "Skip indiv review. Move straight to group review" (or "...and group review... to class review") rather than just naming whatever comes next as if nothing had been skipped. This is new: ticket 337 already carried the underlying data for these two pathway shapes (`splitOnCard`, `toClassReview`) but the card's wording never called out that anything had been bypassed. Pulled into a pure function, `nextStageOnCard` (`lib/pathway.ts`), so the skip/no-skip branching is unit-tested without a component-test harness (this repo has none — `lib/*.test.ts` only).
- **Moving questions out of group review gets a second, narrower confirm step — Back / Confirm move only, the live tick list still underneath it.** Carson: "it's VERY important we get this right — huge implications on teacher control." First draft of the confirm heading read "Added Q7, Q10 to class review" — past tense, stated as already decided. Carson caught this mid-session: "'added Q7, Q10 to class review' is deciding for the teacher, instead of aiding in deciding... want the suggestion... to be obvious, but not decided for teacher." Fixed by keeping the heading forward-looking ("Move Q7 and Q10 out of group review and into class review?") and keeping the row list live and tickable underneath it — nothing is settled until "Confirm move" is actually pressed. Pulled into `moveConfirmSentence` (`lib/splitReview.ts`), a pure string function, so the exact wording (including the two variants below) is pinned by a test rather than left to eyeballing a screenshot.
- **The `emptyAfter` (every group would end up empty) case keeps the pre-tick, but never states the skip as something already done.** Considered dropping the pre-tick for this higher-stakes case (it removes group review from the pathway entirely, not just 2 of 10 questions), but the actual defect was the tense, not the pre-tick: "Skip group review — nothing would be left there. Q7 and Q10 got the fewest right — add any others before confirming?" states the mechanical consequence of the teacher's own ticks as fact (that part isn't being decided in this sentence) while keeping the question selection itself open. Whether the move separately adds class review to the pathway is folded into the same sentence in both cases (never a smaller separate note) — the whole point of the second stage is naming every consequence before the irreversible press.
- **Two new pathway shapes reuse the ticket-337 plumbing rather than getting new trigger code.** "Skip individual review" (individual review off, group review still ahead) already rode on `splitOnCard`, sourced from `talliesSoFar` (first submissions) because `afterCorrections` requires individual review to exist. "Skip group review" (group review off entirely) already rode on `toClassReview`. Both fire from the existing `close-to-finishing` trigger (during individual working) — no new `DecisionKind`, no new due-condition in `lib/decision.ts`. The card's rendering is the only thing that changed for these two shapes: the next-stage line now says the skip in words, and the `toClassReview` note was reworded from a wordy fraction-laden sentence to a short pointer, "Q7 and Q10 got the fewest right — worth covering in class review," with no tick UI at all (Carson: this is deliberately non-binding, a "help the teacher when they get to setup" pointer, not a decision).
- **Card widens from 400px to 480px.** Sized for the split rows (whole question + figure thumbnail + count), the tightest content the card ever holds, not for the now-lighter passive display — the extra room just gives that lighter content more breathing space in the same box.

**Context.** A `/grill-me` session (2026-09-16) worked through the full redesign before any code was written; the transcript is the source for every wording decision above, including the mid-session correction on "Added...".

**Alternatives considered.**
- *Build a second, later trigger point for "skip group review," firing right when individual review ends, instead of reusing the early `close-to-finishing`/`toClassReview` moment.* Carson's own framing ("reachable either from individual working... or from individual review") reads as wanting the notice at the actual transition. Not built: the close-to-finishing card, once stored, lapses the moment the class leaves "working" (`lessonDecision`/`closeView`'s `lapsed` check), so a pathway with individual review on and group review off currently only ever shows this pointer early, during working — never again once individual review is underway. Building a true second axis of "close to finishing" trigger (one for working→next, one for individual-review→next) is a materially bigger change than this ticket's scope, and the data and pointer text shown early are still correct, just early. Flagged as a known gap, not silently fixed by reinterpreting the requirement.
- *Remove the pre-tick for the `emptyAfter` skip case, requiring the teacher to tick every question manually with only a visual "most missed" flag.* Considered directly in response to "deciding for the teacher." Rejected once the actual defect was isolated to tense: the pre-tick was never the problem (an unpressed suggestion is not a decision), the sentence stating it as already done was. Removing it would have been a bigger, unrequested interaction change to fix a wording bug.
- *Fold the emptyAfter/addsClassReview notes into small text under the row list, as the old `data-decision-note` paragraph did.* Rejected for the confirm step specifically: naming every consequence in the one sentence the teacher reads right before the final press is the entire point of a second stage: a note easy to skip under a long tick list defeats it.

**Tradeoffs.**
- **The skip-group-review pointer can go stale.** Because it is only ever shown as the early "close to finishing working" moment (never re-raised once individual review starts, per the trigger gap above), a teacher who dismisses or tucks it away won't see it again with fresher counts once individual review is done. It is explicitly non-binding text, not a decision, which limits the damage, but it is a real gap, not a design choice — recorded in `FUTURE_FEATURES.md`.
- **`nextStageOnCard` and `moveConfirmSentence` are the only two pieces of this redesign with direct unit-test coverage.** Everything else (the two-stage confirm's button wiring, Later/Change hiding, the live tick list staying interactive under the confirm heading) was verified end-to-end against a running production build (a real browser, three pathway shapes, screenshotted) rather than with component tests, because this repo has no component-test harness (`vitest.config.ts` only includes `**/*.test.ts`). That E2E pass is not committed as a repeatable check — it was a one-off CDP script run against the demo's clock, warped via `Date.now()` patching to avoid real wall-clock waits.

**Defense.** Every piece of wording Carson flagged as consequential — the skip statements, the confirm sentence, the emptyAfter variant, the non-binding pointer — is now a pure, named function with a docstring and a test pinning its exact output, not prose embedded in JSX that could drift on the next unrelated edit to the card. The two new pathway shapes needed zero new trigger logic because ticket 337 had already computed the right evidence for both; this redesign only had to stop hiding that work behind a wordier sentence and a missing pre-tick-vs-decided distinction.

## 2026-09-16 · "Your turn", "On your own" and "Skip to the set" all confirm before moving on unfinished; one gate mechanism, not four (ticket 353)

**Decision.** Ticket 349's single-purpose "not finished" confirm (`confirmFor`, one gate: "Next skill") generalizes to a `Gate` of four — `"worked" | "completion" | "done" | "skip"` — each with its own readiness flag and its own message, sharing one `pending`/`asking`/`press` mechanism in `app/student/screens/PracticeScreen.tsx`. "Your turn" and "On your own" now render in their pinned corner for the *whole* step (ticket 349 only rendered them once ready), greyed out until ready, asking first on an early press rather than acting immediately (ticket 349's explicit "no confirm" call for these two) or doing nothing (the reducer's prior silent no-op). "Skip to the set" gets the same corner confirm unconditionally, since skipping is by definition leaving something unfinished. Confirming "Your turn"/"On your own" needed a new capability the reducer didn't have: `warmup/next` gains `force?: boolean`, bypassing its two readiness guards only when explicitly set.

**Context.** Carson, immediately after ticket 349 shipped (2026-09-16): "'your turn' & 'on your own' still not working -- want them in teh bottom right corner of the the center column during whole working period, just greyed out until last step is done. same functionality as 'next skill' -- confirm you want to move on. let's ALSO do this for 'skip to the set' -- confirmation gate." This directly reverses ticket 349's own decision (recorded there) that these two buttons should act unconfirmed, "diff functionality from the 'next skill' thing" — a real, stated change of mind on the same feature within the same session, not a misunderstanding to reconcile.

**Alternatives considered.**
- *Keep the reducer's silent no-op and just add a confirm dialog that, on "yes", does nothing because the guard still blocks it.* Rejected outright: a confirm whose "yes" button doesn't work is worse than no confirm.
- *Four separate `useState` calls, one per gate, copy-pasting ticket 349's `confirmFor`/`confirmLeave` shape.* Works, but the "drops on skill switch, drops once ready" logic is identical across all four and would need to be re-derived (and re-tested) four times. One `Gate`-keyed `pending` object with one derivation (`asking`) keeps that logic in one place while still letting each gate carry its own readiness and message.
- *A `disabled` attribute on "Your turn"/"On your own" instead of `aria-disabled`, matching a literal reading of "greyed out."* Rejected for the same reason ticket 349 rejected it for "Next skill": a truly disabled button eats the click, so the student never gets the ask.
- *Compute `workedReady`/`completionReady` by having `WorkedStep`/`CompletionStep` expose their internal `seen`/`state.done` via a callback prop, instead of recomputing them independently in `PracticeScreen.tsx`.* Rejected: `PracticeScreen.tsx` already has everything needed to compute both directly (`w.exampled`, and `warmupCompletion(session)`, which already exists in `lib/session.ts` for exactly this purpose) — a callback-prop plumbing exercise would duplicate what a direct read already does simply, and would leave `WorkedStep`/`CompletionStep` needing to know about greying/confirming at all, which ticket 349 had already moved out of them.

**Tradeoffs.** `PracticeScreen.tsx` now computes the same readiness the reducer's guards compute, in a second place (`workedReady`/`completionReady` mirror `lib/session.ts`'s own checks) — a small duplication, but the alternative (deriving them from the reducer's internal state some other way) would need new exports for no real gain, and both sides are covered by tests that would catch drift. The four-gate `press` mechanism is one more layer of indirection than a single boolean, which costs a `Gate` type and a small `Record` lookup for four short-lived UI states — judged worth it once a repeated shape (grey until ready, ask before skipping ahead) applies to four controls instead of one.

**A lint false positive, not a design tradeoff, is written up in `architecture/353-warmup-confirm-all.md`** (the `react-hooks/purity` rule flagging a `Date.now()`-touching closure stored as a value in an object literal, worked around with plain-string lookup tables and an inline `if`/`else` in the confirm button's own handler) — noted there rather than here since it shaped the code's exact shape, not the feature's behaviour.
...
**Context.** No keyboard-driven or continuously-running "simulation" existed before this ticket (confirmed by searching the whole codebase for ArrowRight/keydown/autoplay/speed/setInterval); the nearest thing is the presenter's click-driven "skip to" pill lists in `lib/demo.ts`. The two lists don't have the same shape. Sam's `SKIP_TARGETS` is a flat, time-ordered walk through the whole lesson — every entry a one-shot jump, safe to repeat automatically. The teacher's three buttons are not a list to walk: "send assignment" only stages Create's last step (the real send needs a press on a different screen, which nothing here can complete on its own); "students done" is the one action that is already, by itself, a repeatable step-the-class-forward primitive; "activity completed" is a deliberate one-press full skip to the end.

**Alternatives considered.** *Step through `TEACHER_SKIP_TARGETS` in its displayed pill order (send → done → completed), the same flat-index approach as Sam's side.* Rejected: three ArrowRight presses would go send → one stage of "done" → the full-skip "completed", jumping to the end of the lesson far faster than holding the key should read as "fast-forward" rather than "skip to the end". *Auto-complete "send assignment" too, so a held key could run the whole teacher flow start to finish.* Rejected: `readyToSend` only stages Create's last step and deliberately does not send (the real flow is: stage it, then the presenter shows the moment of sending by pressing Create on a different screen, ticket 272); there is no pure jump that finishes that without also faking a screen transition the presenter is meant to narrate live. *Hardcode `ownsArrowKey` to name `DuePicker` and `useReorder` specifically.* Rejected: `e.defaultPrevented` already tells every listener downstream in the bubble chain that something upstream claimed the key, for free, and doesn't need updating if a third ArrowRight consumer is ever added.

**Tradeoffs.** The teacher's held ArrowRight can't run the demo completely hands-off from a blank classroom — the presenter still has to send the set by hand once. Sam's side never reaches the homework jumps (`HOMEWORK_SKIP_TARGETS`), which only exist after a separate manual +Homework press; both gaps are recorded in `FUTURE_FEATURES.md` rather than quietly left unexplained. `teacherDoneAdvances`'s idempotent-stop check relies on `teacherSkip("done", ...)` returning the exact input references once nothing is left to do (`completeLesson`'s early return) — correct today, but it would silently stop detecting "nothing changed" if that function ever switched to always returning a fresh object, so the unit test pins the reference-equality behaviour, not just the boolean result.

**Defense.** Each side's ArrowRight behaviour matches what its own button already means: a flat bookmark list gets walked in order, a genuinely repeatable stage-advance gets repeated, and a step that structurally can't be finished without a screen the presenter narrates live is left alone rather than faked. The accelerating hold (`holdStepDelayMs`) is a named, tested pure function rather than a magic setTimeout chain, and the defer-to-`e.defaultPrevented` mechanism means this feature can't regress `DuePicker` or `useReorder` even as those evolve.

## 2026-09-16 · The decision card gets bigger, on purpose, and gives up a constraint from ticket 335 to do it (ticket 355)

**Decision.** Carson, once ticket 351 was live: "put 'indiv review' in the light blue... make the whole thing bigger. want text bigger, want the pop up to take up more space. like double in size approximately." Two changes:

- **The next-stage line's destination stage name gets `StagePill`'s own light-blue look** (`bg-standout-soft`/`text-ink`), via a small `StageBadge` wrapper — the same visual language the top-right pathway strip already uses, so a stage named mid-sentence reads as the same stage, not a plain word. Only the *destination* stage gets it (the one the class is headed to); a stage named as skipped-over in the same sentence stays plain text, since it isn't where anything is going.
- **The card widens 480px → 680px**, and every size on it scales with it: headline, body copy, split rows, buttons (`size="lg"`), and the Change view's stops (a new additive `card-lg` size on the shared `PathwayStop`, so Create's own pathway line is untouched).

**Context.** Ticket 335 set the card's original size (400px, later 480px in ticket 351) specifically so it would **never cover a roster row** on Class View at 1280px width — the roster's right edge sits 437 layout px from the window's right edge there, so the card's 16px inset plus its width had to stay under that line. Measuring live against a production build (`/teacher/a/pset-6/class`, 1280×800): ticket 351's 480px width was *already* over that line by about 59 layout px — an uncaught regression from that ticket, not something this one introduced. At 680px the overlap is roughly 260 layout px: the roster's rightmost columns (SET score, avatar) sit behind the card while it's open.

**Alternatives considered.**
- *Cap the width at whatever clears the roster (≈421 layout px) and call that "bigger."* Rejected: that's *smaller* than the 480px the card already was, directly contradicting what was asked, in service of a constraint Carson wasn't referencing and may not even remember exists.
- *Make the card's width responsive — narrower when it would cover the roster, full-size elsewhere.* The more "correct" long-term fix, but a materially bigger change (conditional sizing logic, re-verifying every screen/viewport it can appear on) than this round's ask justified. Deferred to `FUTURE_FEATURES.md`.
- *Silently ship the bigger card without mentioning the overlap.* Rejected: ticket 335's constraint was deliberate product decision, not an implementation accident, and dropping it silently would look like a regression nobody chose, next time someone hits it.

**Tradeoffs.** While the card is open on Class View at 1280px, several roster rows' SET-score and avatar columns are covered. The card is dismissible (Later/Keep/Change all close or advance it) and the roster is otherwise fully visible before and after, so the exposure is temporary and narrow — but it is real, and it will recur on this screen every time this card is due, until either the width comes back down or the responsive fix above gets built.

**Defense.** Carson asked for a specific, concrete size increase twice, after already seeing and approving the card's shape — that's a clear, informed request, not an ambiguous spec to second-guess against an old constraint from a different ticket. Shipping it as asked and logging the tradeoff (here, in the ticket, and in `FUTURE_FEATURES.md`) keeps the decision visible and reversible, rather than either quietly under-delivering on the ask or quietly eroding a constraint nobody flagged as changed.

## 2026-09-16 · "← Edexia Classroom" pins to the top of the teacher scroll region instead of scrolling away (ticket 356)

**Decision.** `BackLine` (the back button plus the pathway strip, Class View and Mistakes) and Groups' bare back button are `sticky top-0` inside `[data-teacher-scroll]`, reusing `Classroom.tsx`'s own pinned-heading pattern (bleed over the frame's top/side padding with `-mx-6 -mt-12 bg-cream px-6 pt-12`, so nothing scrolling under it shows through and scroll 0 is unchanged) rather than a new mechanism. Class View's roster head, already `sticky top-0` in its own right since ticket 167, now reads its `top` from `--backline-h` — a `ResizeObserver` on `BackLine` publishing its own rendered height, divided by the frame's zoom — instead of `0`.

**Context.** Carson, looking at the Mistakes tab scrolled down: "make it so that the whole left side doesn't jump around AT ALL. want to be able to see ← Edexia Classroom in all views. yeah just make it stay" — then, asked to confirm scope, "same idea for other review modes, as well" (Class View, Mistakes, Groups; every pathway stage). The first pass pinned `BackLine` alone and shipped with a real collision: Class View's roster head is *also* `sticky top-0` in the same scroll region, so once both were stuck they froze over the same pixels rather than stacking — caught by this ticket's own headless-Chrome screenshot before merge, not by inspection.

**Alternatives considered.**
- *Move `BackLine` out of the scrolling content entirely, into `TeacherChrome`'s own non-scrolling `<header>` as a second row (a `subheader` prop).* Sidesteps sticky-vs-sticky collisions by construction — nothing in the header ever competes with an in-page sticky element for the same pixels, matching the codebase's own stated reason for avoiding sticky in the frame's chrome (`TeacherChrome.tsx`: "a sticky bar rode the bounce with the page"). Not taken: it changes `TeacherChrome`'s API and the vertical rhythm between the nav bar and the eyebrow/H1 below it (the gap `BackLine` used to sit inside would have to be rebuilt to match, rather than reused as-is), for a problem this ticket's actual scope — Class View, Mistakes, Groups — has exactly one instance of (the roster head), fixable in place.
- *Give `BackLine`'s wrapper a higher `z-index` than the roster head's, so it visually wins the collision.* Cheaper, but wrong: the roster head would then be hidden entirely behind `BackLine`'s own padding while stuck, not stacked below it — the column labels (Algebra, Functions, …) would disappear while scrolling instead of staying visible, which is the opposite of this ticket's goal.
- *Hardcode the roster head's offset to a fixed pixel value matching `BackLine`'s current height.* Simpler than a `ResizeObserver`, but silently wrong the next time `BackLine`'s content changes height (the pathway strip's stacked notes above/beside the current stage's pill already vary by stage, per `BackLine.tsx`'s own docstring) — a static number would drift out of sync with no build-time signal.

**Tradeoffs.** A `ResizeObserver` plus a zoom-corrected CSS custom property is more machinery than a plain `top-0` for what reads, on screen, as "two rows stack instead of one." It is the same pattern `Classroom.tsx` already uses for its own pinned region (`--classroom-pinned`), so it is not a new idiom in this codebase, but it is now duplicated rather than shared — `Classroom.tsx`'s version publishes the var for a scroll-margin use, `BackLine.tsx`'s for a sticky-`top` use; extracting a common "publish my own height, corrected for zoom" hook was judged premature for two call sites with different consumers. `WholeClassSetup` and `TeacherCompare` (both `TeacherChrome` pages) currently render no back control at all, and `AssignmentProvider`'s "not created yet" fallback still renders a bare, unpinned `BackToClassroom` — none of the three were the screen this ticket was asked about, and none are a tab a teacher scrolls a long list on; left alone and recorded in `FUTURE_FEATURES.md` rather than folded in silently.

**Defense.** The pinned row's own bleed padding is exactly `Classroom.tsx`'s existing pattern, not a new one, so scroll 0 renders pixel-identical to before on all three tabs (verified against a real production build, not just the sticky mechanism in isolation). The roster-head collision was a real defect this ticket introduced and then found and fixed within the same pass, not a hypothetical guarded against in the abstract — the fix (`--backline-h`) is the smallest change that makes the two elements agree on where the pinned region actually ends, which is the one piece of information a static number could never keep current.

## 2026-09-16 · Held ArrowRight now speeds up time within a stage, not stage-to-stage jumps; scaled at the clock layer, not per-screen (ticket 357, supersedes ticket 354)

**Decision.** Ticket 354's ArrowRight-jumps-between-stages is fully removed and replaced with ArrowRight-speeds-up-time: holding the key runs the demo's shared clock at 10× real time — `useNow()`/`useFrameNow()` in `lib/store.ts`, the only two places any live-updating surface reads "now" — until release. The scaling happens once, at that clock layer (`demoNow()`/`setDemoTimeScale`, with the pure accumulation math pulled out to `lib/demoClock.ts`); nothing downstream (`classReadiness`, `playBoard`/`simulatedRunAt`, `tryAgainShowing`, `dueDecision`, ~30 consuming components) needed to change, because none of them call `Date.now()` themselves — they all already take `now` as a plain argument from one of the two clock hooks.

**Context.** Carson, after seeing 354 running in the browser: "this has the unit of jumping ahead be stages. i really want functionality within a stage. holding arrow key as like 1 second becomes 10 seconds when right arrow key is held." Asked directly whether a tap should keep jumping stages while only the hold's behaviour changed, Carson chose the simpler cut: ArrowRight only ever controls time speed now, tap or hold, and the "skip to" pills go back to being mouse-only shortcuts.

**Alternatives considered.**
- *Scale time only for the screen currently mounted (e.g. pass a `speed` prop into `ClassWaitScreen`, `GroupBoardScreen`, etc.).* Rejected: would need updating every one of the ~30 call sites that read `useNow()`/`useFrameNow()`, each threading a scale value through its own `now`-consuming pure functions, for a result identical to changing the one shared clock those hooks already are.
- *A discrete "warp forward by N seconds" step, similar to ticket 354's repeated jumps but stepping the clock instead of the stage.* Rejected: doesn't match "1 second becomes 10 seconds" — that's a continuous rate while held, not a series of discrete jumps, and a stepped warp would still show the same jerky "teleporting" quality Carson was moving away from.
- *Keep ticket 354's stage-jump on tap, add time-scaling only on hold, on the theory that a quick tap and a long hold naturally read as different gestures.* Was offered directly as a question; Carson chose to drop stage-jumping entirely rather than keep two behaviours on one key.

**Tradeoffs.** The demo's virtual clock can now sit meaningfully ahead of real wall-clock time after a hold, and that offset does not decay — releasing the key only stops it growing, it doesn't rewind (matching "fast forward," not "preview"). `resetSession` ("Reset demo") now also has to reset the clock's scale and offset, a second piece of state to keep in sync with the lesson reset that didn't exist before this ticket; missing it would have meant a freshly reset demo reading as already ahead of itself. The once-a-second clock's sampling means fast-forwarded countdowns and "N of M arrived"-style counts advance in visibly larger jumps than they would at 1× (10 virtual seconds land in one tick instead of ten) — an accepted consequence of reusing the existing 1 Hz clock rather than raising its resolution just for this.

**Defense.** Verified end-to-end against a real production build over CDP, not just the pure clock math in isolation: on Sam's class-wait screen, 3 real seconds unheld advanced classmates' arrivals by 1, the same 3 seconds held advanced them by 8 (~10×, matching the fixture's arrival spacing), and releasing dropped straight back to the real-time rate. Choosing the clock layer as the seam means this ticket's entire diff is `lib/demoClock.ts` (new), a dozen lines in `lib/store.ts`, and a rewritten `lib/arrowHold.ts` — every existing time-driven screen in the app got the feature for free, and none of them can regress it by reading time some other way, because none of them ever could.

## 2026-09-16 · Force submit and the count move below the current pill, taking real room instead of hiding in the header (ticket 358, reverses ticket 345's placement)

**Decision.** `PathwayPills`' `above` prop is replaced with `below`: force submit and "n/total done" now render under the current stage's pill (pill, then force submit, then the count — top to bottom) instead of over it, in that internal order reversed from ticket 345's count-then-button. The visible stack itself is still `absolute`, centred on the pill by the same `left-1/2 -translate-x-1/2` trick ticket 345 used, just hung off `top-full` instead of `bottom-full` — but a new width-less, fixed-height sibling (`data-stage-below-spacer`, 42px, matching the stack's own measured height) sits after it in normal flow, and that spacer is what actually grows the `<li>`, the pathway strip, and the back line under it. `PathwayPills`' `<ol>` switched from `items-center` to `items-start` so that growth on the current stage's `<li>` can never shift any other pill's top edge.

**Context.** Carson (2026-09-16): "right now, force submit is above the pathway... it gets lost in the header. move it below so it's more obvious." Ticket 345 deliberately hung the stack in the 48px of dead padding above the back line specifically so it took no layout room; that was the fix for a different complaint (the strip reading as one long list). This ticket is the direct opposite ask — visibility over invisibility — and there is no equivalent dead space below the pill to hide in; the roster head and page content sit right there. So this ticket is also the first thing to actually exercise `--backline-h` (ticket 356's `ResizeObserver`) for a *height change*, not just a one-time measurement: the mechanism already existed to let the roster head and stuck content reposition when this row grows, this is the first ticket that makes it grow.

**Alternatives considered.**
- *Put the whole stack in flow (no absolute positioning at all): pill, then force submit, then count, stacked as three ordinary block children of the `<li>`.* Tried first, and rejected after measuring it against the pending countdown: "handing in · 0:47 · Cancel" is wider than the plain "force submit" button, and an in-flow stack's width participates in the `<li>`'s own sizing inside the `<ol>`'s flex row — pressing force submit would have widened the current stage's `<li>` and shoved every pill after it sideways. Confirmed with a CDP script driving a real production build: before the fix, the three pills after "indiv working" moved right by the width difference the instant force submit was clicked; after switching to the absolute-plus-spacer approach, all four pills' rects were pixel-identical between the button and pending states.
- *Keep the badge (ticket 335's decision dot) on the pill's bottom-right corner as ticket 345 left it, vs. moving it back to the top-right now that nothing stands there.* Left alone — the ask was specifically about force submit's position, not the dot's, and moving it back is a separate, unrequested cosmetic call. Flagged here and in `tickets/358-…` rather than made silently.
- *Hardcode the roster head's (and anything else stuck under the back line) offset to account for the new taller row.* Not needed: `--backline-h` (ticket 356) already recomputes off a live `ResizeObserver`, so nothing downstream had to change to pick up the new height.

**Tradeoffs.** Force submit being lost in the header is now definitively fixed, but at the literal cost ticket 345 was written to avoid: the back line grows while the current stage has a count, and everything below it — the eyebrow, the title, the roster head — moves down by that amount. That is the point of this ticket, not a side effect, but it does mean the Class View/Mistakes header is now taller during any stage with force submit than it was after 345, and shorter again on Class Review or once the lesson ends (no stack). The fixed 42px spacer height assumes force submit's pending and non-pending states stay height-matched (already true, ticket 345's `STACK_SLOT`); if a future ticket ever let that row's height vary, the spacer would need to grow with it instead of staying a constant.

**Defense.** Verified against a real production build over CDP at 1280×800: all four pathway pills keep an identical top edge and identical left/right edges whether force submit shows its button or its pending countdown; `--backline-h` and the back line's own rendered height are unchanged across that same transition (42px is exactly reserved, not approximately); the roster head's measured top tracks the grown back line on both Class View and Mistakes; `npm run check:laptop` passes all 76 route/size combinations at 1280×800 and 1440×900 with no horizontal overflow. The absolute-plus-fixed-spacer split (rather than a naive in-flow stack) is the smallest change that gets "takes real room" without reintroducing exactly the sideways-shift bug ticket 345's own zero-room design happened to sidestep for free.
