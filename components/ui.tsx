...
export function Card({ children, className = "", tone = "paper" }: { children: ReactNode; className?: string; tone?: "paper" | "soft" | "plain" }) {
  const base =
    tone === "soft"
      ? "bg-accent-soft/60 border-accent-line"
      : tone === "plain"
        ? "bg-transparent border-line"
        : "bg-paper border-line shadow-card";
...
export function Button({
  ...
  return (
    <button
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2 text-[13.5px] font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${v} ${className}`}
      {...rest}
    >
      {children}
    </button>
