 ...
 */
export default function ClassReviewExamples({ problem, examples, onGoTo, stacked = false, student = false }: { problem: string; examples: string[][]; onGoTo: (leaf: LeafId) => void; stacked?: boolean; student?: boolean }) {
  return (
    <div className={`mt-2 grid gap-3 ${stacked ? "grid-cols-1" : ""}`} style={stacked ? undefined : { gridTemplateColumns: `repeat(${examples.length}, minmax(0, 1fr))` }} data-class-review-examples={examples.length}>
      {examples.map((lines, i) => (
        <div key={i} className="min-w-0" data-class-review-example={EXAMPLE_LETTERS[i]}>
          ...
          <div className="-mt-1">
            <WorkLines problem={problem} texs={lines} onGoTo={onGoTo} narrow unmarked student={student} />
          </div>
        </div>
      ))}
    </div>
