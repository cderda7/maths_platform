...
export function sessionScore(session: Pick<StudentSession, "lines" | "answers">, problems: readonly Problem[]): number {
  return problems.filter((p) => {
    const verdicts = (session.lines[p.id] ?? []).map((l) => evaluateLine(p.id, l.tex));
    if (verdicts.length === 0 || verdicts.some((v) => v.verdict === "wrong")) return false;
    return (session.answers[p.id] ?? "").trim().length > 0 || verdicts.some((v) => v.verdict !== "unclear" && v.answer === true);
  }).length;
}
