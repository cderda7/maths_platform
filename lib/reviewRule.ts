 ...
 *
 * - a **habit** (the sheet shows a gap, for that student on that set, in the category the slip counts under)
 *   stays wrong;
 * - a **one-off** slip (its mistake made on that one problem of the set, and the sheet's habit for it naming
 ...
 * last try otherwise, the first habit-holder's working in seating order. When the group's rework checks, every
 * member still wrong on the problem reads as fixed in group review, a habit included (`conflict`: the rules
 ...
 */
export type ReviewBasis = "one-off" | "repeated" | "habit" | "unhelped";
...
export interface ReviewSlip {
  ...
  /** The sheet's habit on this problem in that category, or null when the sheet names none there. */
  habit: { text: string; problems: readonly number[] } | null;
}
...
export interface RuleCase {
  ...
  helpers: string[];
  /** The rules disagree (a habit on a problem the group's rework checked): the group's version decided. */
  conflict: boolean;
}
...
const STRICT: Record<ReviewBasis, number> = { "one-off": 0, repeated: 1, unhelped: 2, habit: 3 };
const RAW: Record<ReviewBasis, ReviewOutcome> = { "one-off": "individual", repeated: "group", unhelped: "wrong", habit: "wrong" };
    ...
    for (const r of members) {
      for (const pid of r.wrong) {
        ...
        for (const s of slipsOf(r, pid)) {
          ...
          const habit = cell.habits.find((hb) => hb.problems.includes(q)) ?? null;
          const slip: ReviewSlip = { name: s.name, category, status: cell.status, habit };
          const repeated = (habit !== null && habit.problems.length > 1) || r.wrong.some((other) => other !== pid && slipsOf(r, other).some((o) => o.name === s.name));
          const helpers = repeated ? members.filter((m) => m.id !== r.id && linesOf(m, pid) !== null && !slipsOf(m, pid).some((o) => o.name === s.name)).map((m) => m.id) : [];
          const basis: ReviewBasis = cell.status === "gap" ? "habit" : !repeated ? "one-off" : helpers.length > 0 ? "repeated" : "unhelped";
