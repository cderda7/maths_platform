...
describe("the roster's progress and the landing", () => {
  ...
  it("the classmates stream in from the start (ticket 189): at Create nobody has handed in; at the end everyone but Chloe (never started, and absent: ticket 250) and Jordan (stalled on Q8)", () => {
    ...
    // Chloe is absent on Problem Set 6 (ticket 250): her row stays, out of the count.
    expect(submittedCount(b, null, now)).toEqual({ submitted: 0, total: CLASS_SIZE - 1 });
    ...
    expect(submittedCount(b, null, now + SKIP_STARTED_AGO_MS)).toEqual({ submitted: CLASS_SIZE - 3, total: CLASS_SIZE - 1 });
    // Sam hands in: the class is past working and every classmate who started has handed in, Jordan too.
    expect(submittedCount(b, sessionAt("feedback"), now)).toEqual({ submitted: CLASS_SIZE - 1, total: CLASS_SIZE - 1 });
  });
