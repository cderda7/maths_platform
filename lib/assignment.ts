...
import { dayLabel, isIsoDay } from "./dueDate";
 ...
 * the goal existed reads the fixture's. The due date is the teacher's pick (ticket 289), else the fixture's.
 */
export interface ActiveAssignment {
  ...
  goal: string;
  /** The day it is due as cards show it ("Thu 10 Sep", ticket 289): the day the teacher picked on Create, else the fixture's. */
  due: string;
}

export function activeAssignment(c: ClassroomState | null | undefined): ActiveAssignment {
  const a = c?.assignment;
  if (!a) return { title: ASSIGNMENT.title, problems: ASSIGNMENT.problems, created: false, newSkills: ASSIGNMENT.newSkills, goal: ASSIGNMENT.goal, due: ASSIGNMENT.due };
  ...
  return { title: a.title.trim() || ASSIGNMENT.title, problems, created: true, newSkills, goal: a.goal ?? ASSIGNMENT.goal, due: isIsoDay(a.due) ? dayLabel(a.due) : ASSIGNMENT.due };
}
