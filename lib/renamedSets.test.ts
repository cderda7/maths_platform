...
describe("the sets renamed (ticket 208)", () => {
  it("Problem Set 2 is Problem Set 6 and Problem Set 1 is Problem Set 5, under their new ids and dates; pset-1 is the real Problem Set 1 now (ticket 211)", () => {
  ...
  it("maps every old id and seeded title to the new one, and leaves anything else alone", () => {
    expect(RENAMED_SET_IDS).toEqual({ "pset-2": "pset-6" });
  ...
  it("a classroom stored before the rename loads under the new ids and titles, with its groups and progress; seating stored under pset-1 stays Problem Set 1's", () => {
    ...
    const old: ClassroomState = { ...moved, draft: { ...generatedDraft(now), title: "Problem Set 2 — Roots of a quadratic" }, assignmentGroups: { "pset-2": liveGroups, "pset-1": ps1Groups }, arrivals: { sam: now } };
    const read = migrateClassroom(JSON.parse(JSON.stringify(old)));
    expect(Object.keys(read.assignmentGroups!).sort()).toEqual(["pset-1", "pset-6"]);
