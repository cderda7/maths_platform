...
import { afterIndividual, groupProblemsOf, reviewByRule, wrongOnOneLine, type RuleOptions } from "./reviewRule";
 ...
 * outcomes, and the sheet's outcomes are the rules' (`reviewByRule`, ticket 332's rule on every set since ticket 338). A
 * one-off slip has a second submission, every line in the set's table, holding line by line exactly where the student fixed
 * it in individual review; no other basis has one. Every question in its group's union after individual review carries the
 * group's one version, and a question outside it none, the same for every member who brought it: holding line by line when
 * solved; when not, a wrong line that shares a real
  ...
  for (const r of everyone) {
    ...
    for (const row of rows) {
      ...
      if (after) {
        ...
        // anything else is never fixed alone.
      ...
      } else if (second.length > 0) out.push(`${where}: a second submission on a pathway without individual review`);
      for (const tex of second) if (evaluateLine(pid, tex).verdict === "unclear") out.push(`${where}: "${tex}" is not in the set's table`);
      const g = later?.group;
      // The group has a version exactly where the question is in its union (ticket 332).
      if (!call) {
      ...
      for (const tex of g.lines) if (evaluateLine(pid, tex).verdict === "unclear") out.push(`${where}: "${tex}" is not in the set's table`);
