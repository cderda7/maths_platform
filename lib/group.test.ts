...
describe("group-phase computation", () => {
  ...
  it("the demo group: the all-correct problems are the quick pass, every question a member still has after individual review the discussion (tickets 278, 332)", () => {
    const g = groupPlan(sessionAt("group"), [], true);
    ...
    // After corrections: Sam 2 (Q7 slipped again, Q9 unfinished), Jordan 4 (Q7 slipped again; Q8–Q10 not reached), Zara 2 (Q7, Q9 slipped again), Liam 9.
    expect(g.discussion.totalWrong).toBe(2 + 4 + 2 + 9);
    expect(g.discussion.perMember).toBe(4);
    // Without individual review on the pathway, first submissions (ticket 278): Sam 6, Jordan 5, Zara 3, Liam 9, the same nine questions.
    const first = groupPlan(sessionAt("group"), [], false);
    expect(first.discussion.problems.map((p) => p.id)).toEqual(g.discussion.problems.map((p) => p.id));
    expect(first.discussion.totalWrong).toBe(6 + 5 + 3 + 9);
  });

  it("the discussion view carries no correctness data: nothing per member, nothing per problem", () => {
    const d = groupPlan(sessionAt("group"), [], true).discussion;
  ...
  describe("a member's problems for the union (ticket 278): wrong, incomplete and not attempted all count; absent members bring nothing", () => {
    it("the demo student: a wrong line counts, Q9 started and left incomplete counts, a problem with nothing on it counts, a finished right one does not", () => {
      ...
      expect(reviewProblemsOf(blank, false)).toEqual(["q1", "q2", "q3", "q7", "q9", "q10"]);
      ...
      expect(reviewProblemsOf(noQ4, false)).toEqual(["q1", "q2", "q3", "q4", "q7", "q9", "q10"]);
      // Nothing written anywhere: every problem.
      expect(reviewProblemsOf(INITIAL_SESSION, false)).toHaveLength(10);
    });
    ...
    it("a classmate: wrong counts, working left past where they finished counts, a problem never reached counts; right inside what they finished does not", () => {
      ...
      expect(recordReviewProblems(m, false)).toEqual(["q2", "q3", "q4", "q5", "q6", "q7", "q8", "q9", "q10"]);
      expect(recordReviewProblems({ done: 0, wrong: [] }, false)).toHaveLength(10);
      expect(recordReviewProblems({ done: 10, wrong: [] }, false)).toEqual([]);
      expect(recordReviewProblems({ done: 10, wrong: ["q3", "q7"] }, false)).toEqual(["q3", "q7"]);
      // The demo group's classmates as the fixture has them.
      expect(recordReviewProblems(CLASSMATE_MAP.jordan, false)).toEqual(["q2", "q7", "q8", "q9", "q10"]);
      expect(recordReviewProblems(CLASSMATE_MAP.zara, false)).toEqual(["q3", "q7", "q9"]);
      expect(recordReviewProblems(CLASSMATE_MAP.liam, false)).toEqual(["q1", "q2", "q3", "q5", "q6", "q7", "q8", "q9", "q10"]);
    });
    ...
    it("the union takes a problem only a not-attempting member brings, and an absent member brings nothing", () => {
      ...
      const union = (mates: { done: number; wrong: string[] }[]) => computePhases(ids, [reviewProblemsOf(s, false), ...mates.map((m) => recordReviewProblems(m, false))]).discussion;
      ...
      expect(groupPlan(s, ["liam"], false).discussion.problems.map((p) => p.id)).toEqual(["q1", "q2", "q3", "q7", "q8", "q9", "q10"]);
      expect(groupPlan(s, ["liam", "jordan"], false).discussion.problems.map((p) => p.id)).toEqual(["q1", "q2", "q3", "q7", "q9", "q10"]);
    });
  });
});
