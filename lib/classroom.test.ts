...
describe("classroom state", () => {
  ...
  it("creating an assignment sets the pathway in force; reset clears it", () => {
    ...
    expect(c.assignment).toEqual({ title: "Set 4", problemIds: ["q1", "q3"], pathway: ["whole-class"], unit: 1, createdAt: 5, startedAt: 5 });
...
describe("active assignment", () => {
  ...
  it("keeps the chosen problems in bank order, drops unknown ids, never returns an empty set", async () => {
    ...
    expect(activeAssignment(empty).title).toBe("PROBLEM SET 6 — ROOTS OF A QUADRATIC");
  });
...
describe("whole-class follow modes and the teacher's pad", () => {
  ...
  it("the teacher's strokes are kept per problem, with undo and clear, and reach the slide", async () => {
    ...
    expect(currentSlide(c)?.teacherInk).toHaveLength(2);
