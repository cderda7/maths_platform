...
import { GROUP_INTRO_PARAGRAPHS, introProgress, introSecondsLeft, tileColumns } from "@/lib/groupIntro";
 ...
 * and the board opens on its own, for every group at once. The bar and the time left are drawn from
 * one `now` that the board screen ticks every frame, so they never disagree (ticket 232).
 */
    ...
    <div className="flex h-full min-h-0 flex-col px-10 pt-6 pb-5" data-group-intro>
      ...
      <div className="mt-auto flex shrink-0 items-center justify-end gap-3 pt-4" data-intro-timer>
        <div className="h-1.5 w-60 overflow-hidden rounded-full bg-cream-deep" aria-hidden>
          {/* Not before the clock's first tick: the hydration render reads 0, which would show a reloaded bar full. */}
          {now > 0 && <div className="h-full rounded-full bg-accent" style={{ width: `${(1 - introProgress(run, now)) * 100}%` }} data-intro-bar />}
        </div>
        <span className="w-10 text-right text-[13px] text-ink-muted tabular-nums" data-intro-left>
          {now > 0 ? mmss(introSecondsLeft(run, now)) : ""}
        </span>
      </div>
    </div>
  );
