...
/**
 * The pill's look, shared with "end lesson" beside it (ticket 273); its size is `FORCE_PILL_SIZE`. Structural only — no
 * border colour, since force submit's own border is purple (`border-accent`, ticket 359) and end lesson's stayed the
 * shared light blue (`border-standout-line`, added at its own call site) rather than following it.
 */
export const FORCE_PILL = "whitespace-nowrap rounded-full border bg-standout-soft font-medium leading-tight text-accent-deep transition-colors hover:bg-standout-line/60 disabled:cursor-not-allowed disabled:opacity-40";
...
export default function ForceSubmit({ stage, session }: { stage: ClassStageId; session: StudentSession | null }) {
  ...
  return pending && advance ? (
    ...
    <button
      type="button"
      className={`${STACK_PILL_SIZE} ${FORCE_PILL} border-accent`}
