...
/**
 * The pathway step, the last before Create: the unit focus to confirm above the review-pathway
 * map (the same map the old create screen has, on a screen of its own). Create is on once the
 * unit is confirmed.
 */
  ...
  return (
    <div className="pb-24" data-pathway-step>
      <div className="mt-8 max-w-[980px] space-y-4">
        <UnitFocus inferred={inferred} reassessed={review.unit ?? null} confirmed={review.confirmed} onConfirm={() => onChange({ confirmed: true })} onReassess={(u) => onChange({ unit: u, confirmed: false })} />
        <Card className="p-6">
      ...
      <div className="fixed bottom-16 right-6 z-30 flex items-center gap-3">
        ...
        <Button size="lg" disabled={!review.confirmed} onClick={onCreate} className="shadow-lift" data-create>
          Create
        </Button>
      </div>
    </div>
