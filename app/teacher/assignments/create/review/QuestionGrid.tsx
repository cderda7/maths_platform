 ...
 * of its tile. With `onMove` a press held on a tile lifts it to drag to another slot, the labels
 * renumbering as the others slide (ticket 150, `useReorder`).
 */
export default function QuestionGrid({ items, onLabel, onMove, animate = false }: { items: GridItem[]; onLabel?: (id: string, d: Difficulty) => void; onMove?: (from: number, to: number) => void; animate?: boolean }) {
  const reorder = useReorder({ count: onMove ? items.length : 0, columns: 5, ignore: "button", name: (i) => `Q${i + 1}`, onMove: (from, to) => onMove?.(from, to) });
  return (
    <>
      <ol className="mt-6 grid grid-cols-5 gap-4" data-questions data-dragging={reorder.drag ? reorder.drag.from + 1 : undefined}>
        {items.map((q, i) => (
          <li key={q.id} className="aspect-square min-h-0" data-question={i + 1} data-origin={q.origin && q.origin !== "typed" ? q.origin : undefined} {...reorder.item(i)}>
            <Tile item={q} index={i} slot={reorder.slot(i)} onRotate={onLabel ? () => onLabel(q.id, nextDifficulty(q.difficulty)) : undefined} animate={animate} />
          </li>
        ))}
      </ol>
      {onMove && (
  ...
  return (
    <div className={`relative flex h-full flex-col rounded-2xl border bg-paper p-5 shadow-card ${marked ? "border-accent-line" : "border-line"}`} data-tile={index + 1}>
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`-mt-0.5 ${arriving ? "label-in" : ""}`} style={arriving ? { animationDelay: `${arrivedAt * 90}ms` } : undefined} data-label-picker>
          {onRotate ? (
            <button type="button" onClick={onRotate} aria-label={`Q${slot + 1} difficulty: ${item.difficulty}. Tap for the next.`} className="rounded-full transition-shadow hover:shadow-card focus-visible:ring-2 focus-visible:ring-accent/40" data-difficulty={item.difficulty}>
