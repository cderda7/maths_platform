...
Pruned 2026-09-16: a handful of items fully built or superseded by a later ticket's own section
were removed (the board pen, the model-solution slide, an old homework-results stub, an old
misread-line stub, a stale homework-model stub — each one's remaining substance already lives in
a newer section here). The big, cross-cutting, not-yet-decided product questions ("raised" rather
than tied to a shipped ticket, plus the two checkpoint-review discussion items) moved out to
`OPEN_QUESTIONS.md`, a sibling file at the repo root, so this file stays scoped to concrete
deferred features tied to specific tickets.
...
- **A harder twist for students who had it right first time.** Everyone does Q** today, including students who had the original right on their first submission (settled at the checkpoint review, 2026-09-15: it replaces write with me, so it must include them). Whether they should instead get a harder variant of Q** was raised then and never confirmed.
...
## Pinning "← Edexia Classroom" everywhere it appears (ticket 356, 2026-09-16)

- **`WholeClassSetup` (class review's own setup screen) and `TeacherCompare` (before/after) render no back control at all.** Neither was the screen this ticket was asked about ("the whole left side" of Class View/Mistakes, then "same idea for other review modes" — read as Class, Mistakes, Groups), and a teacher doesn't scroll a long list on either today. Deferred: give both a real way back (and pin it, for consistency) if either grows content long enough to scroll.
- **`AssignmentProvider`'s "not created yet" fallback still renders a bare, unpinned `BackToClassroom`.** Same component pattern Groups had before this ticket, but the fallback's own content is short enough that it never scrolls in practice. Deferred: fold it into the same pinned treatment if that page ever grows a scrollable body.
- **`--backline-h` and `Classroom.tsx`'s own `--classroom-pinned` are two copies of the same "publish my rendered height, corrected for the frame's zoom" pattern**, one feeding a sticky `top`, the other a scroll-margin. A shared hook was considered and dropped for two call sites with different consumers — worth extracting if a third pinned region needs the same trick.

## Holding ArrowRight speeds up time within a stage (ticket 357, supersedes ticket 354, 2026-09-16)

- **The once-a-second clock's sampling makes fast-forwarded counts and countdowns jump in chunks rather than tick smoothly.** At 10× held, ten virtual seconds land in a single 1 Hz tick instead of ten separate ones, so "N of M handed in" can jump by several at once. `useFrameNow`'s every-frame clock (the group intro's bar, ticket 232) doesn't have this problem. Deferred: raising the once-a-second clock's resolution just for this would cost more re-renders on every ordinary "updated Ns ago" label across the app for a cosmetic smoothness gain only visible while fast-forwarding.
- **The 10× scale is a fixed constant (`DEMO_FAST_FORWARD_SCALE`), not adjustable or rate-limited.** No acceleration ramp (ticket 354 had one for its discrete steps; this ticket's continuous scale doesn't need one to satisfy "1 second becomes 10 seconds," but a presenter wanting an even faster skim has no way to ask for it short of holding longer). Deferred: revisit if 10× proves too slow or too fast in real use.
- **No visual indicator that fast-forward is active.** Holding ArrowRight gives no on-screen cue (an icon, a subtle border) that time is running fast versus something on screen just changing quickly on its own. Deferred: the only feedback right now is the sped-up movement itself, judged sufficient for a presenter-only control who is holding the key on purpose.
- **Time only ever speeds up, never rewinds or slows below 1×.** A presenter who fast-forwards past a moment they meant to show has no way back except a full Reset demo. Deferred: out of scope for "speeds up," and rewinding a clock that other pure state (like `classroom.group`'s already-committed simulated actions) was derived from would need real thought about what "undo" even means here.
