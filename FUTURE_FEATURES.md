...
- **A walk-around mode for the teacher on an iPad or iPhone.** In an in-class lesson the teacher walks the room; a glanceable "who needs me now" list (help requests, stuck students, a rework that broke a problem that was right) would suit a phone better than the laptop's full views. Deferred 2026-09-14 by the user ("add to F_F — the ipad/iphone mode"): the laptop views come first. Asked for again at the 2026-09-15 checkpoint review. What the phone view would be built from now exists: Where students are (ticket 315) and its dark purple check-in time (ticket 328) are the "who needs me now" signal, and the student work panel (ticket 316) is what a tap on a name would open. Open questions: whether the phone shows only students due a check-in or the whole room, whether it can do anything (mark a check-in done, send a hint) or only read, how it follows the teacher between stages (warm-up, help, group review), and whether the laptop stays open at the front as the controller.
...
- **Correcting a wrong mark or a misread line.** With a real OCR and grading backend, a misread will happen in a pilot's first lesson; the teacher needs to override a mark and the student to say "that's not what I wrote". Out of scope 2026-09-14: the demo has no OCR and assumes a symbolic grading engine. Scoped in full under "That's not what I wrote" (Checkpoint review, 2026-09-15).
...
- **Update (checkpoint review, 2026-09-15): Carson wants the pair mode; not ticketed yet.** Ticket 310's Q* and Q** already exist for every Problem Set 6 question, ticket 311 checks a written line, and ticket 315's split is the laptop's frame, so the mode is mostly assembly; it is the design input for ticket 320 (class review on the split). The vote is out (Carson: students already committed in their submissions, and the teacher has the spread). One problem runs in three steps: (1) the real examples, screens frozen, as today; (2) the teacher reveals Q* line by line on the board and every iPad, talking over it, with the board pen to annotate; (3) "your turn": every iPad unfreezes onto Q** on the student's own canvas, the laptop shows Where students are on the left and the mistake cards by misconception filling in on the right, and the teacher closes it. Proposed and not yet confirmed one by one: it replaces write with me rather than adding a third mode; everyone does Q**, including students who had the original right first time; Q** is the whole problem, not its blanks; lines are marked when the student presses check, not while writing; it never changes the set score (ticket 285) but gives the report a measured "right after class review" in place of "covered in class review" (ticket 282); the teacher ends step 3 (leaning on "teacher judgement over defaults"). Still open: a harder twist for students who had it right; pairs for sets made on Create (generated Q*/Q**, entry under ticket 310); what the board shows during step 3 (the question only, no counts, ticket 202).
...
- **Pressing a completed or missed cell.** Since ticket 326 a press shows the demo placeholder "HW insight scoped in FUTURE_FEATURES" for 2.5 s; what the press would really open is scoped as "Sam's cells open his own homework insight" under ticket 324.
...
- **Reading back a finished homework.** A completed or missed homework has no screen (only an open one renders); the HW1 and HW2 cells could open a read-only view of what was in them and what was done. Deferred by the ticket. Since ticket 326 those cells show the demo placeholder on a press; the view is scoped as "Sam's cells open his own homework insight" under ticket 324.
...
- **Sam's cells open his own homework insight.** Since ticket 326 (2026-09-15) a press on Sam's completed, missed or still-in-the-Future cell shows the same placeholder, "HW insight scoped in FUTURE_FEATURES" white on dark grey for 2.5 s; the open homework's cell keeps opening the homework screen. What the press stands in for, on his side and about him only: (1) *A finished homework read back*: the completed cell opens a read-only Homework 1 in the homework screen's order (his own problems under their sets, then the teacher's ten), each with his answer lines as marked, right first time or fixed, and the day he handed it in; once "Answering homework" (ticket 293) exists these are his real lines, until then there is nothing to read back but the list and the day. (2) *A missed homework, framed forward*: the missed cell opens Homework 2's list with what he did and did not reach, and says where its own problems went, phrased as already taken care of, never as more to do: the problems that carried read as part of the homework he has now ("in Homework 3", linking to it), and any leftover dropped because Homework 3 already holds that skill is simply not listed as owed. It never shows a "covered" count, a list of dropped problems, or anything that reads as more work without credit (the user's rule, ticket 294's decision log: "it will make them feel like they have more work to do without getting credit for it"); the teacher's side of the dropped judgement stays in the teacher's insight. The teacher's ten he never did are not listed as a debt either (ticket 294's "The gap from a missed homework's teacher problems" is the teacher's call). (3) *A waiting homework*: the Future cell's press could say when it opens (after Problem Set 6's lesson) and which sets it covers, without listing problems, since its contents freeze only when it opens (ticket 292). (4) *His homework history*: each homework in a row with its due and handed-in days (ticket 307's dates) and, once answering exists, his right-first-time share; no class comparison, no names, no difficulty tags (student side). Deferred: students cannot answer homework yet ("Answering homework", "Done and undone marks", ticket 293), so (1) and (4) would show only a list and dates, and the student-facing wording for (2) needs the user's sign-off before it ships. Replaces the earlier "Sam's cells opening their own read-back" (cells were unpressable).
...
## Sam's homework cell placeholder (ticket 326, 2026-09-15)

- **What the press opens.** Scoped as "Sam's cells open his own homework insight" under ticket 324; the placeholder stands in until then.
...
## Class review markup (ticket 330, 2026-09-15)

- **Pen colours and a highlighter.** Carson chose the pad's navy for now; red for a slip, blue for the right line, or a translucent highlighter over a line would let a mark say what it means. The marks are SVG strokes, so a colour or width per mark is one field on `Markup`. Deferred: one pen was asked for.
- **An eraser that takes one mark.** Undo takes the last stroke and Clear takes everything; a teacher who wants to remove a mark from three strokes ago has to undo the two after it. A press-to-erase tool would hit-test the SVG paths. Deferred: not asked for.
- **Strokes that span pieces of the slide.** A long arrow from A to C is pinned to the one anchor nearest its middle, so its far end can land slightly off on another surface. Splitting a stroke into pieces pinned to the anchors it passes, or pinning its ends separately, would keep both ends on their maths. Deferred: the common marks (a circle, a tick, a cross, an underline) sit on one piece.
- **Scrolling a long example column with a finger on the board.** The slide takes the pen outright on the board and the laptop, so a column taller than its slot scrolls only with a wheel or trackpad. A pen-or-hand switch, or two-finger scroll, would give touch both. Deferred: the columns are fitted to fit, and none of the demo's examples overflow.
...
## Past sets under the after-individual-review rule (ticket 338, 2026-09-16)

- **Finished records' group lines.** Each finished record's `groupStatus` ("Group review done · explained Q7") predates ticket 332's union. Amber's on Problem Sets 1 and 2 describe a group review it now sat out, and other tables' lines name questions they no longer work. No screen shows them for a finished set; if a past set's Groups tab ever shows members' lines, rewrite them from the union (deferred: nothing reads them).
- **A group that sat out, on the past set's Groups tab.** A finished set's Groups tab still lists amber on Sets 1 and 2 like any group; a "sat out, nothing left after corrections" mark there was not asked for.
- **More than one unsolvable question per set.** Each of Sets 1–5 keeps exactly one question nobody at a table could explain (mint's Q10 on four of them). A richer spread (a different table, or an easier question a table still could not do) would need more second submissions to slip again; kept minimal for now.
...
## The decision card confirms one stage, not the whole pathway (ticket 351, 2026-09-16)

- **Reconsidering whether class review itself is on or off, at a skip transition.** Carson, scoping this ticket: "for rn we'll just not mess wit the class review — we'll assume that a teacher that chose that initially won't go back on their decision to do it." A teacher who switched class review off at Create time is never asked to reconsider it from a skip-individual-review or skip-group-review card; only the questions moved into it (which forces it on, per ticket 337's existing rule) can add it back. Deferred: a live decision about turning class review on where it was deliberately switched off is a bigger, separate decision surface than this ticket's scope, and the assumption that the original choice stands held up through every scenario worked through in the grilling session.
- **A genuinely later trigger for skip-group-review.** On a pathway with individual review on and group review off, the "worth covering in class review" pointer only ever shows on the early close-to-finishing card (during individual working) — it rides on evidence ticket 337 already computed there, `toClassReview`. The close-to-finishing card lapses the moment the class leaves "working" (`lib/decision.ts`'s `closeView`), so the pointer is never re-raised once individual review is actually wrapping up, and its counts can read stale by then. A true second trigger axis — "close to finishing individual review" as well as "close to finishing working" — would fix this, but is a materially bigger change than this ticket's scope (a new due-condition and evidence path in `lib/decision.ts`, not just a rendering change). Deferred: recorded honestly in `architecture/351-next-stage-confirm.md` rather than quietly left unmentioned.
- **A component-test harness.** Every piece of consequential wording in the decision card (the skip statements, the confirm sentence, the emptyAfter variant) had to be pulled out into pure `lib/` functions to get any automated test coverage at all, because `vitest.config.ts` only runs `**/*.test.ts` — there is no jsdom/React Testing Library setup for asserting on rendered component output or simulating a button press directly. The two-stage confirm's actual button wiring (Later/Change hiding, Back's non-destructiveness) was verified with a one-off CDP script against a running production build instead, which is not a repeatable check anyone else can re-run later. Deferred: worth revisiting if decision-card-shaped UI keeps growing new interaction states that pure functions can't fully cover.

## Holding ArrowRight speeds up the demo simulation (ticket 354, 2026-09-16)

- **ArrowRight never reaches the homework jumps.** Neither Sam's SkipTo nor the teacher's TeacherSkipTo lets a held key walk into `HOMEWORK_SKIP_TARGETS` ("send homework", "homework open") — those only ever appear after the teacher's own manual +Homework press elsewhere in the real UI, a separate, deliberate action a held key shouldn't run ahead of. Deferred: revisit if the homework jumps ever become reachable without that manual press first.
- **The teacher's "send assignment" step can't be automated past Create.** It only stages Create's last step (`readyToSend`); actually sending needs the presenter to press Create by hand on a different screen, which a keyboard shortcut on the SkipTo strip can't reach into. So ArrowRight there does nothing until a set has actually been sent (by mouse), then only repeats "students done with current stage". Deferred: would need Create's own screen to expose a keyboard-drivable "send" path, out of scope here.
- **No visual indicator of which pill is "current."** Neither SkipTo's mouse clicks nor ArrowRight's stepping highlights the pill last landed on; the only feedback is the app itself moving (the page navigating, the class stage changing). Deferred: minor, and adding a selected-state to buttons that have never had one felt like scope creep for a presenter-only control.

## The decision card's width no longer respects ticket 335's roster-clearance rule (ticket 355, 2026-09-16)

- **A responsive/adaptive width for the decision card.** Ticket 335 sized the card so it would never cover a row of Class View's roster at 1280px width (roster's right edge sits 437 layout px from the window's right edge there). Ticket 355 widened the card 480px → 680px on Carson's explicit ask ("make the whole thing bigger... like double") without preserving that guarantee — at 680px the card now covers the roster's rightmost columns (SET score, avatar) while it's open at 1280px. A proper fix would size the card down (or reposition it) only when it would actually collide with content beneath it, rather than a fixed width regardless of viewport or screen. Deferred: a materially bigger change (conditional sizing logic, re-verifying every screen/viewport this card can appear on) than the size bump itself; see `DECISION_LOG.md`, 2026-09-16, for the full tradeoff.
- **A note in passing: ticket 351's 480px width already broke the same rule by ~59 layout px before this ticket touched it.** Nobody had caught it (it wasn't part of 351's own E2E pass, which didn't happen to check the Class View roster specifically). Worth a general lesson: any future width change to this card should re-check this constraint explicitly, not just re-run `check:laptop` (which checks page overflow, not this overlay).
