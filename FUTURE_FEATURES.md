...
- **Neutral mistake names on the per-set screens.** The Mistakes tab's and examples' short names still say "guessed pair, not expanded back" (`data/evaluation.ts`, `data/pset3–5/evaluation.ts`), "middle terms guessed", "perfect square's sign rushed". They are teacher-facing labels on the chips, whose widths are tuned, so renaming needs a sweep of the Mistakes tab. Deferred: outside the holistic reports.
