...
- **"Continue tomorrow" as the normal case.** Raised as a risk that ten problems plus three review stages and a reflection won't fit a period. Not pursued 2026-09-14: the user thinks it fits a 70-minute lesson (Queensland senior periods are commonly 70 minutes, e.g. The Gap SHS), so it stays in its existing deferred entry.
- **A one-page "what happened, what to do Monday" brief that works if the teacher does nothing.** Proposed from the outside critique's "every feature spends teacher time". Rejected 2026-09-14: the user sees teacher judgement as the product's strength; guidance comes from raising decisions when due (ticket 255), not from deciding for the teacher.
- **Moving teacher decisions to automatic defaults (groups, board examples, diagnostic picks, pathway).** Rejected 2026-09-14 for the same reason: "the system decides just leads to a reliance on defaults we arbitrarily set instead of teacher judgement."
- **Cutting mechanics (stars, detective mode, the confidence survey, recorded vs unrecorded diagnostics).** Proposed as a delete pass; rejected 2026-09-14 by the user, who wants to keep them.
