...
- **Tiles.** `DraftQuestion` gains `uploaded?: true` (not `origin`, which the review step already uses for changed/added questions), `confirmed`, `sourceId`, `name`, `thumb`, `page`, `label`. A shimmer tile per file at the end of the grid while its request runs; each draft becomes a tile inserted before its file's marker, `data-unconfirmed`, tinted `standout-soft` with a `standout-line` border, the source thumbnail 40 px in the **bottom**-right corner (the header row keeps the label and the controls), keep (✓) and discard (×) always showing on an unconfirmed tile. An uploaded tile's text is the model's stem on one line and its TeX on the next (`draftText`; Q18); `lib/mathInput.ts` passes a line through untouched when it is already TeX (a backslash command or a brace group) and reads `$…$` in the stem as explicit inline maths, so editing either line re-renders.
...
- [x] `/teacher/assignments/create` in fixture mode: the ghost tile reads "Type a question, or drop a picture" with Upload beneath (focused on load and blurred); dragging a file over shows the overlay with no tile moving; dropping `one-problem.png` shows a shimmer tile then one tinted tile with the thumbnail; `worksheet.png` gives four tiles in reading order with their sheet numbers; Upload via the file input does the same
- [x] discard removes one tile (the undo line offers it back); ✓ keeps one; "Add 3" clears the tint on the rest; Continue with an unconfirmed tile present carries it to the review step confirmed
- [x] a 21-image drop adds twenty and the bar names the one left out; an 11 MB file, a PDF and a text file are refused by name
- [x] a pasted screenshot (synthetic clipboard event with a file) goes through the same path; pasted text still splits into tiles
- [x] with no key and no flag: the message tile "Upload needs the model. Not configured." with Dismiss; nothing else on the screen changes
- [x] editing an uploaded tile's TeX re-renders it; typed tiles unchanged; a reload keeps the uploaded tiles with their thumbnails
- [x] vitest (496), eslint, tsc, `next build`, headless click-through (`upload171.mjs`, 36 checks at 1400×1000 with the bar checked at 1280×800, 7 in plain mode)
