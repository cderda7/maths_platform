...
- [ ] with no key and no flag, `POST /api/extract` answers 503 `not-configured` before any stream; a bad body 400; a source over ten MB or a sixth PDF 413
- [ ] with `EXTRACT_FIXTURES=1`, each of the three fixtures streams its drafts one per line in reading order with `page` and `label` set where they apply, and `done` last; a typed text fixture returns its one draft; an unknown source returns the "no fixture" draft
- [ ] with a key, the one-problem fixture returns one draft whose TeX typesets in KaTeX (checked by hand once, recorded in the architecture note)
- [ ] vitest, eslint, tsc, `next build`, a node script against the running route in fixture mode (`extract170.mjs`)
