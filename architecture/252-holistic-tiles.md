...
| File | What it does |
| --- | --- |
| `data/habitTags.ts` | `HABIT_TAGS`: per student and category, the habits the story sheet words differently from set to set, each with one short label ("signs in the wrong brackets" for Sam's PS4 and PS5 wordings). Authored, never matched by similarity; a habit worded alike on every set needs no entry. `habitTagLabel(student, category, text)`: the label, else the habit's own words. |
| `data/habitTags.test.ts` | Every text is that student's habit in that category, word for word; each tag gathers two wordings or more over two sets or more; a text in one tag only. |
| `lib/holisticTiles.ts` | `holisticTiles({ classroom, session, now })` (the twenty in the class order) and `holisticTile(student, …)`, over `holisticView` (ticket 251): name, initials, the sheet's line, the student's page href; the habits collapsed by tag per category with the sets each shows on, kept when on two sets or more (`RECURRING_SETS`), most sets first; strengths, the categories secure on every set that assessed the student in them (a set not assessing the category, a set they were away for, and the live set where nothing of theirs is seen yet are passed over). |
...
| `app/teacher/students/HolisticTiles.tsx` | The page: ← Edexia Classroom, eyebrow, "Holistic Assessment", twenty tiles three to a row (a row one height). A tile is one link: avatar, name, →, the line, then each category's tags ("label · n sets", sets on hover) and a Strengths row, or "No habit on two sets yet". Waits for the session batch and the clock; restores the scroll once drawn. |
...
```
 data/story.ts  STORY[student].cells[category][set].habits[{ text, problems }]
      │                                   data/habitTags.ts  HABIT_TAGS[student][category] = [{ label, texts[] }]
      ▼                                            │
 lib/holistic.ts  holisticView (251)               │ habitTagLabel(student, category, text)
      │  columns[{ label, finished }] · rows · habits[category → text → refs]
      ▼                                            ▼
 lib/holisticTiles.ts  holisticTile / holisticTiles({ classroom, session, now })
      │   habits ─► collapse by label, sets = union of refs, keep ≥ 2 sets, most sets first
      │   rows   ─► strength: every result secure (skip —, absent, not seen on the live set), at least one
      │   { student, href, summary, habits[category → tags[{ label, sets }]], strengths[] }
      ▼
