...
> Since ticket 276 (`architecture/276-patterns.md`): the side column's habits are called patterns (`Patterns`, `data-holistic-side="patterns"`), with no header over them, only recent ones surface, and a set head has no live pill.

## Files touched

| File | What it does |
| --- | --- |
| `app/teacher/students/HolisticPage.tsx` | `Body` holds the open cell and picked skill (press elsewhere and Escape close them) and lays the header and grid left, the side column right; `useFillHeight` sizes the side column to the scroll region's bottom. `Grid` draws sets newest first, coloured `Cell`s as buttons, and a positioned box the flyout lays over. `Patterns` run down the side column inside `FitHeight`. |
...
```
 lib/assignments.ts rosterEvidence ──┬──► lib/holistic.ts holisticView   (cells, patterns)
 ...
 │ Grid (PS6 … PS1) ◄277                          │  Patterns in FitHeight ◄277            │
