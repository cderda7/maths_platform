...
| `app/student/screens/GroupDebrief.tsx` | Hint "take a moment to reflect". New `HoldRing`: wraps Next, measures itself with a `ResizeObserver`, draws two pixel-sized `<rect>`s (`rx` = half the height) 4 px outside the button, the second dashed by `progress`; nothing is drawn once `progress` is 1, which is the same moment `holdOver` enables Next (follow-up 2026-09-10) |
   ...
   attempts: [ SLIPS.q7,   ──▶     line×3 · check ✗ · +3.5 s ·   ──▶    group/line, group/check …
            ...
            GroupBoardScreen ◀────────────┘
              ...
              check ✓ ──▶ resolved ──▶ GroupDebrief
                ...
                PEER_DEBRIEF_MS = 16 000  │     holdOver ──▶ Next enabled, hint gone, ring gone
                (StudentApp: when a peer   └─ <HoldRing progress>   (svg only while progress < 1)
...
reflect" shown until Next enabled at 11 s. Follow-up run on port 3141: ring 68 × 44 px with dash
offset 100 → 61.5 → 21.5 at 0.3 s, 4 s and 8 s; at 11.5 s Next enabled, no `[data-ring]` in the
DOM, the button a plain solid pill (zoomed crops checked). Then through Zara's Q2 and Jordan's Q3 to Liam's Q7:
