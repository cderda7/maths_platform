...
```
 ...
 PracticeScreen (ticket 27): sequence[warmup.step] on the pad · script = its model steps
   chip strip: [done □✓][done □✓][current ■][todo □]  · "Next skill →" ──▶ warmup/skill-done ──▶ step + 1 … last ──▶ working
...
fractions" → "Got it. Let's get started.", chips light blue → "warm up on these →"
→ fractions on the pad with the four-chip strip → "Next skill →": fractions ticked, monic dark
blue → worked example → "Try one more" → the follow-up with "Next skill →" → two more skills →
