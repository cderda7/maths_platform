...
| `app/student/screens/PracticePrompt.tsx` | `PromptModal` is for detected triggers only ("A natural next step"); asking for help skips it. `PracticeOverlay` is `PracticePad` over the working screen: title "On its own", the skill as a dark-blue chip, "Back to Qn →" as footer and finished button. `HelpPicker` lists only the current problem's own moves that have a practice, all in the light-purple row style, and closes on a tap outside |
...
```
 WorkingScreen ── "I need help" ──▶ HelpPicker(problemLeaves(p) ∩ practiceFor ≠ null) ── pick ──▶ help/request ──▶ overlay = leaf · overlayRun = INITIAL_RUN (no prompt: the student asked)
               ── wrong line ──▶ recordMistake(group) ── 2nd ──▶ prompt { leaf: practiceLeaf(tag) }   (null for a whole-task leaf: no prompt)
 PromptModal ("A natural next step") ── Yes ──▶ prompt/accept ──▶ overlay = leaf · overlayRun = INITIAL_RUN
                                              │
                                              ▼
