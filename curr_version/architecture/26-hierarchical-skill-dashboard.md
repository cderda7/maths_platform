...
| File | What it does |
|---|---|
| `data/taxonomy.ts` (+ test) | The Methods taxonomy as code: seven categories in canonical order → groups → leaves, `version`, ids derived from the tree (`CategoryId`, `GroupId`, `LeafId`), lookups (`categoryOf`, `groupOf`, `groupsOf`, `leavesOf`), names in three forms, `resolveLeaf` (unknown id → one warning, null), `unitOf`; `FLAT_CATEGORIES`/`isFlat` (Unit Focus is drawn with two layers: its skills hang straight off the category and the group level is not drawn) and `categoryLabel(c, unit)` ("Unit 1": the grid's header stays UNIT for consistency between classes, but the unit's name sits to the right of the Unit dot while a student's drill is open and to the left of it, grey, in a column view; the reports name the node Unit 1) |
...
| `app/teacher/TeacherLive.tsx` | Pathway card at the top of the right column, stages stacked with a drawn down arrow between; ways to open: a category dot (that category's tree under the dot, work beside or beneath; double-click opens every group and skill of that category), a double-click on a category header (the column opens for every student at group level, again with skills, again to close; a single click on any header closes it, deferred 350 ms so the two clicks inside a double-click do not close the column first; no ring on the dots in a column view), a click anywhere else on the row (every category's groups, each tree fitted inside its own column, skills on demand; a second click closes), a double-click (every group and skill at once). The per-student practice card ("Sam Okonkwo · Practice · … · declined") was removed from the right column on 2026-09-09; practices remain in the report. Category dots are centred in their columns with the heading centred over each dot; in the all-categories views a tree starts under its dot and may run until the next column's dot (the last to the grid's edge), text measured with the page's real font; the live student's pill reads "Q3 in progress" while working (else "live", or "not started") with the stage word in lowercase under the name; classmate notes are structured (`notes: { text, problems[] }`), rendered lowercase with a hanging indent, and clickable: a comment opens the student's full expanded view with only the skills behind its problems coloured (`restrictTo` rolls groups and categories up from those leaves alone; `leavesBehind` collects a problem's leaves plus the leaf of any wrong line), the same comment again closes; in a column view every other category's dots are blanked; columns = categories the assignment touches; 15 px dot buttons with accessible names; one expanded row at a time; classmates through the same evidence path |
