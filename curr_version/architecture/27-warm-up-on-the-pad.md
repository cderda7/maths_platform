...
| `app/student/screens/PracticeScreen.tsx` | The warm-up as the working layout: problem · pad · Read as, "On to the set" always on the right. `HelpMenu` (hint / worked example / video dead link). The example replaces the pad; complete → "Try one more →" → split pane: finished example over the follow-up on the left, clean pad in the middle |
