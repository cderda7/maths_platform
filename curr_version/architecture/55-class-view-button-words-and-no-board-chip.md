...
| File | What it does |
|---|---|
| `app/teacher/TeacherLive.tsx` | Header buttons: `all` = see skills (+ full breakdown unless flat); `levels` = only the expanded button when the expanded level is open; the active one reads "close" (`aria-label` "Close … for every student"). With the expanded level open the lone close uses `STACK_TALL`: `h-[calc(2*(1.375*11px_+_6px)_+_4px)]`, two buttons plus the 4 px gap, text centred by grid. `RowGroup` is now a `<tbody className="group/row">` per student and the single `<tbody>` is gone, so `group-hover/row` covers the row and its drill row. `data-see-skills` reads "close" and calls `setOpen(null)` whenever `isOpen`, in any mode. `BoardIndicator` no longer imported or rendered |
...
```
   ...
   │                    full breakdown open ▶ ┌───────┐  (one button,   │
   │                                          │ close │   two buttons   │
   │                                          └───────┘   tall)         │
   │                                                                      │
...
column closes from the lone "close", whose box measures the same top and height as the two-button stack; with a column open the row actions are hidden until the mouse
