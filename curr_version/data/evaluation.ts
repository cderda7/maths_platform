...
export const EVALUATION: Record<string, Record<string, LineVerdict>> = {
  q1: {
    ...
    "(x - 2)(x - 3) = 0": ok(T(MONIC), "Factorised"),
    "x = 2 \\;\\text{or}\\; x = 3": ok(T(NFL, QUAD), "Null factor law"),
    "(x + 2)(x + 3) = 0": wrong(
      T(MONIC),
      "Factorised",
  ...
  q2: {
    ...
    "x = -2 \\;\\text{or}\\; x = 1": ok(T(LIN), "Solved each factor", true),
  },
  q3: {
    ...
    "x = 9 \\;\\text{or}\\; x = 4": ok(T(LIN), "Solved each part", true),
  },
  q4: {
    "a = 3,\\; b = -5,\\; c = -1": ok(T(QUAD), "Identified a, b, c"),
    "b^2 - 4ac = 25 + 12 = 37": ok(T(DISC), "Discriminant"),
    "x = \\dfrac{5 \\pm \\sqrt{37}}{6}": ok(T(QUAD, FRAC), "Quadratic formula"),
  ...
  q8: {
    "x = 1 \\;\\text{or}\\; x = 3": ok(T(FEAT, ZERO), "Read from the graph"),
    "1 - 4 + 3 = 0 \\;\\checkmark": ok(T(EVAL, ZERO), "Checked by substitution"),
    "x = -1 \\;\\text{or}\\; x = -3": wrong(
      T(FEAT),
      "Read from the graph",
  ...
  q9: {
    ...
    "h = -9 + 18 = 9": ok(T(FEAT), "Greatest height 9 m"),
    "h = 6": wrong(
      T(FEAT),
      "Greatest height",
  ...
  q10: {
    ...
    "\\text{The graph never meets the x-axis}": ok(T(CONCL, SKETCH), "In context"),
    ...
    "\\text{The graph crosses the x-axis twice}": wrong(
      T(CONCL),
      "In context",
