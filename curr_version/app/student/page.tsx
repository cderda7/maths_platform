...
export default function StudentHome() {
  ...
  return (
    <IpadStage>
      <StudentChrome crumb={ASSIGNMENT.className}>
        <div className="grid h-full min-h-0 grid-cols-[440px_1fr]">
          <aside className="min-h-0 overflow-y-auto border-r border-line px-9 py-8">
            ...
            <div className="mt-3 rounded-xl border border-accent-line bg-accent-soft/60 px-4 py-2.5 text-[14.5px] font-medium text-ink">
              {target.name}
            </div>
          ...
          <section className="px-9 py-8">
            <div className="flex items-end justify-between">
              ...
              <Button size="lg">Start the set</Button>
            </div>
              ...
              {ASSIGNMENT.problems.map((p) => (
                <li key={p.id}>
                  <Card className="h-full p-5">
                    ...
                    <p className="mt-3 text-[13.5px] text-ink-soft">{p.stem}</p>
